...
public class OnionManager: NSObject, @unchecked Sendable {
    ...
    public nonisolated(unsafe) static let shared = OnionManager()
    
    private var reachability: Reachability?
    private var torDelegate: OnionManagerDelegate?
    
    // Show Tor log in iOS' app log.
    private static let TOR_LOGGING = false

    private nonisolated(unsafe) static let torBaseConf: TorConfiguration = {
                ...
                if success {
                    ...
                    completeObs = self.torController?.addObserver(forCircuitEstablished: { established in
                        if established {
                            ...
                            self.torController?.getSessionConfiguration({ configuration in
                                self.torDelegate?.torConnFinished(configuration: configuration!)
                            })
                        }
                    ...
                    progressObs = self.torController?.addObserver(forStatusEvents: {
                        ...
                        if type == "STATUS_CLIENT" && action == "BOOTSTRAP" {
                            ...
                            self.torDelegate?.torConnProgress(progress)
