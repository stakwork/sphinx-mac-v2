 ...
 */
export interface StepDef<TType extends string = string, TInput extends z.ZodTypeAny = z.ZodTypeAny, TOutput extends z.ZodTypeAny = z.ZodTypeAny, TServices = unknown> {
    type: TType;
    description?: string;
    input: TInput;
    output: TOutput;
    ...
    source?: string;
    run: (cfg: z.infer<TInput>, ctx: StepContext<TServices>) => Promise<z.infer<TOutput>>;
}
...
export type RunEventType = "step.start" | "step.end" | "step.error" | "step.retry" | "step.skipped"
 ...
 *  no side effects re-executed. Never a fake `step.end` (honest timings). */
 | "step.replayed" | "run.start" | "run.end" | "run.error"
 ...
 */
export declare function withAccessedNodes<T>(output: T, nodes: Array<AccessedNode | null | undefined>): T;
/** The nodes a step output was marked with (see `withAccessedNodes`), else
 *  undefined. */
export declare function accessedNodesOf(output: unknown): AccessedNode[] | undefined;
 ...
 * ```
 */
export declare function defineStep<TType extends string, TInput extends z.ZodTypeAny, TOutput extends z.ZodTypeAny, TServices = unknown>(def: StepDef<TType, TInput, TOutput, TServices>): StepDef<TType, TInput, TOutput, TServices>;
/**
 ...
 * ```
 */
export declare function step(id: string, type: string, config: Record<string, unknown>, options?: StepOptions & {
    depends?: string | string[];
    when?: boolean;
}): Step;
/**
 ...
 * ```
 */
export declare function flow(name: string, opts: {
    input: z.ZodTypeAny;
    steps: Step[];
    params?: Record<string, unknown>;
}): Flow;
