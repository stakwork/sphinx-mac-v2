 ...
 */
const SECRET_TOKEN = (name) => `{{secret:${name}}}`;
export function emptyCassette() {
    return { entries: [] };
}
/**
 ...
 */
export function withCassette(services, opts) {
    const { mode, cassette } = opts;
    const secretsService = opts.secretsService === undefined ? "secrets" : opts.secretsService;
    ...
    const redact = (v) => {
        ...
        return deepReplace(v, (s) => {
            ...
            for (const [name, value] of secretValues) {
                if (value)
                    out = out.split(value).join(SECRET_TOKEN(name));
            }
            return out;
        });
    };
    /** Handle one wrapped call (non-secrets services). */
    const handleCall = async (key, fn, args) => {
        const redactedArgs = redact(args);
        if (mode === "replay") {
            const idx = cassette.entries.findIndex((e, i) => !consumed.has(i) && e.key === key && argsEqual(e.args, redactedArgs));
            if (idx === -1) {
                throw new Error(`cassette replay: no recorded call for "${key}" with args ${stableJson(redactedArgs)}`);
            }
            consumed.add(idx);
        ...
        if (svc && typeof svc === "object") {
            return new Proxy(svc, {
                get(target, prop) {
                    ...
                    const bound = member.bind(target);
                    if (isSecrets && method === "get") {
                        return wrapSecretsGet(bound);
                    }
                    return (...args) => handleCall(`${name}.${method}`, bound, args);
                },
            });
        }
        return svc;
...
function deepReplace(v, fn) {
    if (typeof v === "string")
        return fn(v);
    if (Array.isArray(v))
        return v.map((x) => deepReplace(x, fn));
    if (v && typeof v === "object") {
        const out = {};
        for (const [k, val] of Object.entries(v))
            out[k] = deepReplace(val, fn);
        return out;
    }
    return v;
}
...
export async function loadCassette(path) {
...
export async function saveCassette(path, cassette) {
