...
export async function listRunSummaries(store, name, limit) {
...
export async function readRun(store, name, runId, fullEvents) {
 ...
 */
export async function searchRunEvents(store, name, pattern, opts = {}) {
    ...
    for (const runId of runIds) {
        ...
        for (const e of events) {
            ...
            matches.push({
                ...
                ...(e.stepType ? { stepType: e.stepType } : {}),
                count,
                snippet: line.slice(Math.max(0, first.index - SNIPPET_BEFORE), first.index + first[0].length + SNIPPET_AFTER),
            });
        ...
        if (matchingEvents > 0)
            perRun.push({ runId, matchingEvents });
    }
    return {
        workflow: name,
        pattern,
...
export function coerceJsonArg(v) {
    if (typeof v !== "string")
        return v;
    const t = v.trim();
...
async function verifyLoaded(deps, name) {
    const fresh = await deps.getRegistry();
    if (fresh[name])
        return { loaded: true };
    const err = await stepLoadError(join(await deps.workspace.materializeCustomSteps(), `${name}.ts`));
    return {
        loaded: false,
        loadError: err ??
...
export async function publishNewStep(deps, name, code, description, publisher) {
...
export async function publishStepVersion(deps, type, code, description, opts = {}) {
    if (deps.publishingEnabled === false) {
        return { error: "Step publishing is disabled (the registry was injected at construction)." };
    }
    const customs = await deps.workspace.listSteps();
    ...
    if (opts.requirePublisher && existing.publisher !== opts.requirePublisher) {
        return {
            error: `Step "${type}" was not published by "${opts.requirePublisher}" (publisher: ` +
...
export function buildAuthoringCapability(deps) {
    const { workspace, store } = deps;
    const explorerDeps = async () => ({ workspace, registry: await deps.getRegistry() });
    const findWorkflow = async (name) => (await workspace.listWorkflows()).find((w) => w.name === name);
    ...
    const notOwned = async (name, verb) => {
        const entry = await findWorkflow(name);
        if (!entry)
            return `Workflow "${name}" not found`;
        if (entry.publisher !== AI_PUBLISHER) {
            return (`Workflow "${name}" is not agent-authored (publisher: ${entry.publisher ?? "none"}) — ` +
    ...
    return {
        ...
        async runStep(type, args = {}) {
            ...
            return runSingleStep(type, registry, deps.services, {
                config: coerceJsonArg(args.config),
                input: coerceJsonArg(args.input),
                params: coerceJsonArg(args.params),
                workspace,
                ...(args.cassette
                    ? {
                        cassette: {
        ...
        async publishWorkflow(name, yaml, description, category) {
            ...
            if (entry && entry.publisher !== AI_PUBLISHER) {
                return {
                    error: `Workflow "${name}" exists and is not agent-authored (publisher: ` +
                        `${entry.publisher ?? "none"}). The meta surface never edits workflows it ` +
                        `didn't publish — choose a new name to author a candidate.`,
                };
            }
            try {
                const result = await workspace.publishWorkflowByContent(name, yaml, description, category, AI_PUBLISHER);
                return {
                    ok: true,
                    name,
                    version: result.version,
        ...
        async runWorkflow(name, input, params, version, opts) {
            ...
            try {
                return await runWorkflow(flow, coerceJsonArg(input) ?? {}, registry, {
                    ...
                    workflowHash: (await workspace.getWorkflowHash(flow.name, version)) ?? undefined,
                });
            }
            finally {
                tracked?.untrack();
