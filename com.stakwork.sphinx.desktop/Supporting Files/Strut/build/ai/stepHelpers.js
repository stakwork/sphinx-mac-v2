 ...
 */
async function inCodeRegistryExtras(deps) {
    const onDiskCustom = new Set((await deps.workspace.listSteps()).map((s) => s.type));
    const libTypes = new Set();
    for (const f of await findLibStepFiles(LIB_DIR, ""))
        libTypes.add(f);
    const extras = [];
    for (const [type, def] of Object.entries(deps.registry)) {
        if (CORE_STEP_TYPES.includes(type))
            continue;
        if (libTypes.has(type))
            continue;
        if (onDiskCustom.has(type))
            continue;
        extras.push({ type, description: def.description });
    }
    return extras;
}
/** Recursively walk LIB_DIR and yield step type names (path minus extension). */
async function findLibStepFiles(dir, prefix) {
    let names;
    try {
...
function normalizeStepsPath(path) {
...
export async function lsSteps(path, deps) {
    ...
    if (root === "lib" && parts.length >= 3) {
        ...
        if (!entries)
            return { error: `Path "steps/lib/${subPath}" not found.` };
        return { entries };
    }
    ...
    if (root === "custom" && parts.length === 2) {
        ...
        return {
            entries: merged.map((s) => s.description ? `${s.type}  — ${s.description}` : s.type),
        };
    }
...
async function listStepDir(dir) {
    let names;
    try {
        names = await readdir(dir);
    }
...
export async function searchSteps(query, deps) {
    ...
    for (const [type, def] of Object.entries(deps.registry)) {
        ...
        if (score > 0)
            results.push({ type, description: desc, score });
    }
 ...
 */
export async function readStepSource(type, deps) {
    if (CORE_STEP_TYPES.includes(type))
        return undefined;
