...
export function formatRunNotification(info, maxOutputChars = 2000) {
    ...
    if (info.status === "success" && info.output !== undefined) {
        ...
        if (json.length > maxOutputChars) {
            lines.push(`Output (truncated, ${json.length} chars total):`, json.slice(0, maxOutputChars));
        }
        else {
...
export function createChatNotifier(opts) {
    ...
    async function launch(chatId, texts) {
        ...
        try {
            ...
            const msgs = texts.map((t) => ({ role: "user", content: t }));
            ...
            if (autoTurns >= opts.maxAutoTurns) {
                console.warn(`[chat ${chatId}] auto-turn cap (${opts.maxAutoTurns}) reached — notification appended, turn NOT launched; waiting for a human message.`);
                live.delete(chatId);
                return;
            }
