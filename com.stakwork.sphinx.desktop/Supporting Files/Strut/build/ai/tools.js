...
import { AI_PUBLISHER, coerceJsonArg, listRunSummaries, publishNewStep, publishStepVersion, readRun, searchRunEvents, } from "../authoring.js";
function runControlTool(controlRun, action, description) {
    return tool({
        description,
...
function runControlTools(controlRun) {
    return {
        cancel_run: runControlTool(controlRun, "cancel", "Cancel a run that is LIVE in this process — e.g. a detached run_workflow you launched with the wrong input, or one you want to stop after seeing partial output in get_run. Cancellation is cooperative: the run stops at its next step boundary and finalizes as cancelled (a [run-notification] follows for chat-launched runs). A run that already finished reports its terminal status instead."),
        pause_run: runControlTool(controlRun, "pause", "Pause a LIVE run at its next step boundary (in-flight steps finish; no new ones start) so you or the user can inspect it with get_run before deciding to resume_run or cancel_run. Only live runs can be paused."),
        resume_run: runControlTool(controlRun, "resume", "Resume a run you paused with pause_run (in-memory; the run continues from where it parked). For a run cut off by a crash/restart, tell the user to use the UI's durable resume instead."),
    };
}
// ── Tools ──────────────────────────────────────────────────────────────────
export function buildTools(deps) {
    ...
    const validate = async (yaml, name) => {
    ...
    const withWarnings = (result, v) => v.warnings.length ? { ...result, warnings: v.warnings } : result;
    return {
        list_steps: tool({
            description: "List contents of a step path, like a filesystem. Valid paths: 'steps' (shows core/, lib/, custom/), 'steps/core', 'steps/lib', 'steps/lib/<namespace>', 'steps/custom'.",
            inputSchema: z.object({
                path: z
                    .string()
                    .default("steps")
                    .describe("Path to list. Defaults to 'steps' (the root). Use 'steps/lib' to see lib namespaces, 'steps/lib/github' to see steps in a namespace, etc."),
            }),
            execute: async ({ path }) => lsSteps(path, deps),
        }),
        search_steps: tool({
            description: "Search for step types by keyword. Matches against the step type name and its description across core, lib, and custom steps. Returns ranked matches.",
            inputSchema: z.object({
                query: z
                    .string()
        ...
        get_step: tool({
            description: "Get details for a specific step type: its input schema fields, and source code for lib/custom steps.",
            inputSchema: z.object({
        ...
        list_secrets: tool({
            description: "List the NAMES of credentials available in the deployment's secret store (e.g. GITHUB_TOKEN, GOOGLE_SERVICE_ACCOUNT_JSON). Returns names + metadata ONLY — never the secret values. Use this before authoring a step that needs auth: reference an existing name in ctx.services.secrets.get(\"NAME\"), and if the credential you need isn't listed, tell the user to add it via the Secrets dialog (the value is never visible to you).",
            inputSchema: z.object({}),
            execute: async () => {
                if (!deps.secrets) {
        ...
        edit_step: tool({
            description: "Publish a NEW VERSION of an EXISTING custom step (e.g. tweak its prompt, logic, or config schema). Same self-contained rules as create_step. Call get_step first to read the current source. Identical content is a no-op; a change increments the version (v1 → v2 → …) and prior versions are kept for rollback. Built-in core/lib steps cannot be edited.",
            inputSchema: z.object({
        ...
        create_workflow: tool({
            description: "Create and publish a NEW workflow from YAML. If the name already " +
            ...
            inputSchema: z.object({
                ...
                category: z
                    .string()
                    .optional()
                    .describe("Optional sidebar grouping label (kebab-case, e.g. an experiment name). Omit to leave uncategorized."),
            }),
            execute: async ({ name, yaml, description, category }) => {
                const v = await validate(yaml, name);
                if (!v.ok)
                    return { error: formatValidationErrors(v), validation: v };
                const { name: finalName, version } = await deps.workspace.createWorkflow(name, yaml, description, category);
        ...
        edit_workflow: tool({
            description: "Publish a NEW VERSION of an EXISTING workflow from YAML. Call " +
            ...
            inputSchema: z.object({
                ...
                category: z
                    .string()
                    .optional()
                    .describe("Optional sidebar grouping label. Only pass to CHANGE the category (to merely re-categorize without editing YAML, use set_workflow_category)."),
            }),
            execute: async ({ name, yaml, description, category }) => {
                const exists = (await deps.workspace.listWorkflows()).some((w) => w.name === name);
                if (!exists) {
                    return {
                ...
                let result;
                try {
                    result = await deps.workspace.publishWorkflowByContent(name, yaml, description, category);
                }
                catch (err) {
        ...
        set_workflow_category: tool({
            description: "Set or clear an existing workflow's sidebar category (the grouping " +
        ...
        list_workflows: tool({
            description: "List all published workflows in the workspace, with each one's active version, all versions, and description. Use this to discover what workflows already exist before creating a new one or referencing one in a subflow.",
            inputSchema: z.object({}),
            execute: async () => {
        ...
        get_workflow: tool({
            description: "Get a published workflow's full YAML source plus its version metadata. Defaults to the active version; pass `version` for a specific one. Use this to read an existing workflow before editing, referencing it in a subflow, or running it.",
            inputSchema: z.object({
                name: z.string().describe("Workflow name"),
                version: z
                    .string()
            ...
            execute: async ({ name, version }) => {
                const entry = (await deps.workspace.listWorkflows()).find((w) => w.name === name);
                if (!entry) {
        ...
        run_workflow: tool({
            description: "Run a published workflow with a given input and return the result. Use this to test workflows you just created. Returns status (success/error), output (on success), error details (on failure), and the runId. " +
            ...
            inputSchema: z.object({
                ...
                input: z
                    .any()
                    .optional()
                    .describe("Input passed to the workflow as a JSON OBJECT (not a string), referenced in step configs via {{ input.* }} — the run subject, e.g. { owner, repo, pull_number }. Use {} if none."),
                params: z
                    .record(z.string(), z.any())
                    .optional()
                    .describe("Optional overrides for the workflow's `params` knobs (prompts, thresholds, sample sizes). Shallow-merged over the workflow's `params` defaults — set just the knobs you want to vary for this trial. Referenced in step configs via {{ params.* }}."),
                version: z
                    .string()
                    .optional()
                    .describe("Optional specific version. Defaults to the active version."),
            ...
            execute: async ({ name, input, params, version }) => {
                ...
                const promise = runWorkflow(flow, coerceJsonArg(input) ?? {}, deps.registry, {
                    ...
                    params: coerceJsonArg(params),
                    controller: tracked?.controller,
                    workflowHash: (await deps.workspace.getWorkflowHash(name, version)) ?? undefined,
                ...
                return {
                    ...
                    runId,
                    workflow: name,
                    note: `Run still executing after ${Math.round(detach.waitMs / 1000)}s — it continues detached in the background. ` +
        ...
        run_step: tool({
            description: "Run a SINGLE step in isolation with a given config + input, and return its output + events — WITHOUT wiring it into a workflow. This is the inner loop for authoring an adapter: create_step → run_step → edit_step → run_step until the output is right. " +
        ...
        list_runs: tool({
            description: "List past runs of a workflow (newest first), each with its status, duration, and timestamps. Use this to inspect a workflow's run history — e.g. to compare experiment runs or find a failing run. Then call get_run for a specific run's full input/output/events.",
            inputSchema: z.object({
        ...
        get_run: tool({
            description: "Get a single run's details: its summary (input, output, status, error, duration) and its event log. By default the event log is slimmed (type/path/duration/error per step, no payloads) to stay token-cheap; set fullEvents:true to include each step's input/output. Use this to debug why a run failed or to read what each step produced.",
            inputSchema: z.object({
                name: z.string().describe("Workflow name"),
        ...
        search_runs: tool({
            ...
            inputSchema: z.object({
                ...
                maxMatches: z
                    ...
                    .positive()
                    .default(50)
                    .describe("Cap on returned matches; scanning stops once reached (truncated: true). Narrow the pattern or run window rather than raising this (default 50)."),
        ...
        ...(deps.graph
            ? {
                graph_query: tool({
                    description: "Run a READ-ONLY Cypher query against the strut graph (Neo4j) and get rows back. Use it to VERIFY what a workflow's graph/* steps actually wrote — count nodes/edges by type, read back exact properties, check edge fan-out — or to inspect graph-backed workspace state. " +
        ...
        ...(deps.webSearch ? { web_search: anthropic.tools.webSearch_20260209({ maxUses: 5 }) } : {}),
    };
}
