...
export function isRegisteredEdge(edge, sourceType, targetType) {
    return STRUT_EDGES.some((r) => r.edge === edge && r.source === sourceType && (WILDCARD_TARGET_EDGES.has(edge) || r.target === targetType));
}
export function edgeKeyFor(edge) {
...
export function composeEdgeKey(pattern, properties) {
    const lower = new Map(Object.entries(properties).map(([k, v]) => [k.toLowerCase(), v]));
    return pattern
        .split("-")
        .map((tok) => {
        if (!lower.has(tok.toLowerCase()))
            throw new GraphValidationError("MISSING_REQUIRED", "edge", `edge_key property ${tok} missing from edge data`, tok);
...
export class EdgeWriter {
    ...
    async update(where, patch) {
        ...
        for (const [k, v] of Object.entries(patch.set ?? {})) {
            if (v === undefined)
                continue;
            if (PROTECTED.has(k))
                throw new GraphValidationError("UNKNOWN_ATTRIBUTE", "edge", "edge stamp is system-managed", k);
            if (!IDENT.test(k))
                throw new GraphValidationError("UNKNOWN_ATTRIBUTE", "edge", "edge property is not a bare identifier", k);
            set[k] = k === "weight" && typeof v === "number" && Number.isInteger(v) ? int(v) : v;
        }
        const remove = [...new Set(patch.remove ?? [])];
        for (const k of remove) {
            if (PROTECTED.has(k) || k === "weight")
                throw new GraphValidationError("UNKNOWN_ATTRIBUTE", "edge", "edge stamp is system-managed", k);
            if (!IDENT.test(k))
                throw new GraphValidationError("UNKNOWN_ATTRIBUTE", "edge", "edge property is not a bare identifier", k);
        }
        if (Object.keys(set).length === 0 && remove.length === 0)
            throw new GraphValidationError("MISSING_REQUIRED", "edge", "nothing to set or remove");
        ...
        return this.bolt.write(async (tx) => {
            ...
            if ("ref_id" in where) {
                if (!where.ref_id)
                    throw new GraphValidationError("MISSING_REQUIRED", "edge", "ref_id is required");
                rows = await txRows(tx, `MATCH (s)-[r {ref_id: $ref_id}]->(t) SET r += $set ${removeClause} ${ret}`, { ref_id: where.ref_id, set });
                if (rows.length === 0)
                    throw new GraphValidationError("NOT_FOUND", "edge", `no edge with ref_id ${where.ref_id}`);
            }
            else {
                const edge = where.edge.toUpperCase().replace(/ /g, "_");
                if (!EDGE_TYPE.test(edge))
                    throw new GraphValidationError("UNKNOWN_TYPE", edge, "edge type must match ^[A-Z][A-Z0-9_]*$");
                if (!where.source_ref_id || !where.target_ref_id)
                    throw new GraphValidationError("MISSING_REQUIRED", edge, "source_ref_id and target_ref_id are required");
                const found = await txRows(tx, `MATCH (source:Data_Bank {ref_id: $src})
           ...
           RETURN r.ref_id AS ref_id`, { src: where.source_ref_id, tgt: where.target_ref_id });
                if (found.length === 0)
                    throw new GraphValidationError("NOT_FOUND", edge, `no ${edge} edge from ${where.source_ref_id} to ${where.target_ref_id}`);
                if (found.length > 1)
                    throw new GraphValidationError("DUPLICATE_KEY", edge, `${found.length} ${edge} edges between those nodes — pass the edge ref_id`);
    ...
    async validateEndpoints(tx, inputs) {
        ...
        const nodes = new Map(rows.map((r) => [r["id"], { labels: r["labels"], uid: r["uid"] }]));
        const out = [];
        for (const i of inputs) {
            const s = nodes.get(i.source_ref_id);
            const t = nodes.get(i.target_ref_id);
            if (!s)
                throw new GraphValidationError("MISSING_REQUIRED", i.edge, `source ${i.source_ref_id} does not resolve`, "source_ref_id");
            if (!t)
                throw new GraphValidationError("MISSING_REQUIRED", i.edge, `target ${i.target_ref_id} does not resolve`, "target_ref_id");
            const st = typeLabelOf(s.labels);
            const tt = typeLabelOf(t.labels);
            if (!st)
                throw new GraphValidationError("WRONG_TYPE", i.edge, "source node has no type label", "source_ref_id");
            ...
            if (isStrutType(st)) {
                ...
                if (!match)
                    throw new GraphValidationError("WRONG_TYPE", i.edge, `Invalid edge type: ${i.edge} (no edge schema ${st}-[${i.edge}]->${tt ?? "?"})`);
                const pattern = match.properties["edge_key"];
                if (typeof pattern === "string" && pattern)
                    edge_key = composeEdgeKey(pattern, i.properties ?? {});
            }
...
function validateEdgeShape(i) {
    if (!EDGE_TYPE.test(i.edge))
        throw new GraphValidationError("UNKNOWN_TYPE", i.edge, "edge type must match ^[A-Z][A-Z0-9_]*$");
    if (!i.source_ref_id || !i.target_ref_id)
        throw new GraphValidationError("MISSING_REQUIRED", i.edge, "source_ref_id and target_ref_id are required");
    for (const k of Object.keys(i.properties ?? {})) {
        if (STAMPS.has(k))
            throw new GraphValidationError("UNKNOWN_ATTRIBUTE", i.edge, "edge stamp is system-managed", k);
        if (!IDENT.test(k))
            throw new GraphValidationError("UNKNOWN_ATTRIBUTE", i.edge, "edge property is not a bare identifier", k);
    }
}
async function mergeEdges(tx, edge, edges) {
    ...
    const out = await txRows(tx, `UNWIND $edges AS e
...
//# sourceMappingURL=edge-writer.js.map