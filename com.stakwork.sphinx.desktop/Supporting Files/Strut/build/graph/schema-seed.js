...
import { THING_SCHEMA, THING_TYPE, STRUT_DOMAIN_LABEL, STRUT_EDGES, STRUT_SCHEMAS, embeddingColumn, searchableAttributes, vectorIndexName, vectorIndexedPairs, } from "./strut-schemas.js";
 ...
 */
export async function schemaStatement(bolt, cypher) {
    try {
        ...
        if (/already exists|cannot be created until|equivalent/i.test(msg))
            return msg.split("\n")[0];
        throw e;
    }
}
...
export async function seedStrutDomain(bolt) {
    const report = {
        mode: "shared",
        createdSchemas: [],
        reconciled: {},
    ...
    const ddl = async (cypher) => {
        const skipped = await schemaStatement(bolt, cypher);
        if (skipped)
            report.skippedSchemaObjects.push(skipped);
    };
    ...
    if (!hadThing) {
        await bolt.run(`MERGE (s:Schema {type: $type}) ON CREATE SET s = $thing, s.ref_id = $ref_id`, { type: THING_TYPE, thing: flattenSchema(THING_SCHEMA), ref_id: randomUUID() });
    }
    ...
    for (const schema of STRUT_SCHEMAS) {
        ...
        const hit = await bolt.run(`MATCH (n:Schema) WHERE toLower(n.type) = toLower($t) RETURN keys(n) AS ks LIMIT 1`, { t: schema.type });
        if (hit.length === 0) {
            await bolt.run(`CREATE (s:Schema) SET s = $flat, s.ref_id = $ref_id`, { flat, ref_id: randomUUID() });
            report.createdSchemas.push(schema.type);
        }
        else {
            const live = new Set(hit[0]["ks"]);
            const missing = {};
            for (const [k, v] of Object.entries(flat)) {
                if (k === "type" || k === "parent" || k === "ref_id")
                    continue;
                if (!live.has(k))
                    missing[k] = v;
            }
        ...
        await bolt.run(`MATCH (child:Schema {type: $child}), (parent:Schema {type: $parent})
       MERGE (child)-[r:CHILD_OF]->(parent)
       ON CREATE SET r.ref_id = $ref_id`, { child: schema.type, parent: THING_TYPE, ref_id: randomUUID() });
        // 4. Per-type (node_key, namespace) uniqueness + node_key range index.
        await ddl(`CREATE CONSTRAINT ${`unique_${schema.type.toLowerCase()}_node_key`} IF NOT EXISTS
    ...
    await ddl(`CREATE CONSTRAINT unique_node_key_global IF NOT EXISTS
    ...
    for (const e of STRUT_EDGES) {
        ...
        if (r === "created")
            report.createdEdgeSchemas.push(`${e.source}-[${e.edge}]->${e.target}`);
        else if (r === "skipped")
            report.skippedEdgeSchemas.push(`${e.source}-[${e.edge}]->${e.target}`);
    }
    ...
    await ddl(`CREATE VECTOR INDEX \`${DOMAIN_VECTOR_INDEX}\` IF NOT EXISTS
     FOR (n:\`${STRUT_DOMAIN_LABEL}\`) ON n.text_embeddings ${VECTOR_OPTIONS}`);
    await ddl(`CREATE VECTOR INDEX \`${GLOBAL_VECTOR_INDEX}\` IF NOT EXISTS
     FOR (n:Data_Bank) ON n.text_embeddings ${VECTOR_OPTIONS}`);
    for (const { type, prop } of vectorIndexedPairs()) {
        await ddl(`CREATE VECTOR INDEX \`${vectorIndexName(type, prop)}\` IF NOT EXISTS
    ...
    await ddl(`CREATE FULLTEXT INDEX \`${DOMAIN_FULLTEXT_INDEX_V2}\` IF NOT EXISTS
     FOR (n:\`${STRUT_DOMAIN_LABEL}\`) ON EACH [${props}]
     OPTIONS { indexConfig: { \`fulltext.analyzer\`: 'english' } }`);
    // 9. Migration ledger stamp so jarvis's runner sees done work, not conflict.
    await ddl(`CREATE CONSTRAINT migration_id_unique IF NOT EXISTS
     FOR (m:Migration) REQUIRE m.migration_id IS UNIQUE`);
    await bolt.run(`MERGE (m:Migration {migration_id: $id}) ON CREATE SET m.executed_at = timestamp()`, { id: STRUT_MIGRATION_ID });
    return report;
}
/**
 ...
 */
async function seedEdgeSchema(bolt, e) {
    const rows = await bolt.run(`MATCH (s:Schema {type: $src}), (t:Schema {type: $tgt})
     MERGE (s)-[r:\`${e.edge}\`]->(t)
     ON CREATE SET r.ref_id = $ref_id
     RETURN r.ref_id = $ref_id AS created`, { src: e.source, tgt: e.target, ref_id: randomUUID() });
    if (rows.length === 0) {
        if (e.note)
            return "skipped";
...
//# sourceMappingURL=schema-seed.js.map