...
export const THING_SCHEMA = {
    ...
    type_description: "The highest-level node in the ontology hierarchy, representing an abstract concept with no direct individual instances",
    title_key: "name",
...
export const THING_INHERITED_ATTRIBUTES = {
    name: "?string",
...
export function typeLabelOf(labels) {
...
export function getStrutSchema(type) {
    return BY_TYPE.get(type);
}
export function isStrutType(type) {
...
export function effectiveAttributes(schema) {
 ...
 */
export function searchableAttributes(schemas = STRUT_SCHEMAS) {
    const out = new Set();
...
export function vectorIndexedPairs(schemas = STRUT_SCHEMAS) {
    const out = [];
    for (const s of schemas)
        for (const p of s.vector_index ?? [])
            out.push({ type: s.type, prop: p });
    return out;
}
// ── Author-time validation ──────────────────────────────────────────────────
const IDENT = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
const EDGE_TYPE = /^[A-Z][A-Z0-9_]*$/;
const ATTR_TYPES = new Set(["string", "boolean", "int", "float", "datetime", "list"]);
/**
 ...
 */
export function assertLibraryWellFormed(schemas = STRUT_SCHEMAS, edges = STRUT_EDGES) {
    const types = new Set();
    for (const s of schemas) {
        if (!IDENT.test(s.type))
            throw new Error(`schema ${s.type}: type is not a bare identifier`);
        if (types.has(s.type))
            throw new Error(`schema ${s.type}: duplicate type`);
        types.add(s.type);
        if (s.parent !== THING_TYPE)
            throw new Error(`schema ${s.type}: parent must be Thing`);
        if (s.domain !== STRUT_DOMAIN)
            throw new Error(`schema ${s.type}: domain must be Strut`);
        for (const [name, t] of Object.entries(s.attributes)) {
            if (!IDENT.test(name))
                throw new Error(`schema ${s.type}: attribute "${name}" is not a bare identifier`);
            if (RESERVED_ATTRIBUTE_NAMES.has(name))
                throw new Error(`schema ${s.type}: attribute "${name}" is reserved`);
            if (GENERIC_NODE_PROPERTIES.has(name))
                throw new Error(`schema ${s.type}: attribute "${name}" is a generic node property`);
            if (!ATTR_TYPES.has(baseType(t)))
                throw new Error(`schema ${s.type}: attribute "${name}" has unknown type "${t}"`);
        }
        ...
        for (const tok of tokens.slice(1)) {
            ...
            if (!t)
                throw new Error(`schema ${s.type}: node_key token "${tok}" is not a declared attribute`);
            if (isOptional(t))
                throw new Error(`schema ${s.type}: node_key token "${tok}" must be required`);
        }
        const declared = effectiveAttributes(s);
        if (s.index.length === 0)
            throw new Error(`schema ${s.type}: index must be non-empty`);
        for (const f of [...s.index, ...(s.vector_index ?? []), s.title_key, s.description_key]) {
            if (!declared[f])
                throw new Error(`schema ${s.type}: field "${f}" is not a declared attribute`);
        }
        for (const f of s.vector_index ?? []) {
            if (baseType(declared[f]) !== "string")
                throw new Error(`schema ${s.type}: vector_index field "${f}" must be a string`);
        }
    }
    for (const e of edges) {
        if (!EDGE_TYPE.test(e.edge))
            throw new Error(`edge ${e.edge}: type must match ^[A-Z][A-Z0-9_]*$`);
        if (!types.has(e.source))
            throw new Error(`edge ${e.edge}: source "${e.source}" is not a Strut type`);
...
//# sourceMappingURL=strut-schemas.js.map