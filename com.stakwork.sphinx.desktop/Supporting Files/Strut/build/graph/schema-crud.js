...
function checkAttribute(type, name, value) {
    if (!IDENT.test(name))
        throw new GraphValidationError("UNKNOWN_ATTRIBUTE", type, "attribute name is not a bare identifier", name);
 ...
 */
export function planNodeSchema(input) {
    const type = String(input.type ?? "").trim();
    if (!TYPE_LABEL.test(type))
        throw new GraphValidationError("UNKNOWN_TYPE", type || "?", "type must match ^[A-Za-z][A-Za-z0-9_]*$");
    if (type === "*" || type === THING_TYPE)
        throw new GraphValidationError("UNKNOWN_TYPE", type, "cannot create the root or wildcard schema");
    if (isStrutType(type))
        throw new GraphValidationError("UNKNOWN_TYPE", type, "Strut's own types are a closed in-code registry");
    const parent = String(input.parent ?? THING_TYPE).trim() || THING_TYPE;
    if (!TYPE_LABEL.test(parent))
        throw new GraphValidationError("UNKNOWN_TYPE", type, `parent must match ^[A-Za-z][A-Za-z0-9_]*$, got ${JSON.stringify(input.parent)}`, "parent");
    const attributes = {};
    ...
    if (rawKey.toLowerCase().startsWith(prefix))
        rawKey = rawKey.slice(prefix.length);
    const tokens = rawKey.split("-").map((t) => t.trim()).filter(Boolean);
    if (tokens.length === 0)
        throw new GraphValidationError("MISSING_REQUIRED", type, "node_key needs at least one attribute token", "node_key");
    for (const t of tokens) {
        if (!declared.has(t))
            throw new GraphValidationError("UNKNOWN_ATTRIBUTE", type, `node_key token ${t} is not a declared attribute`, "node_key");
    }
    const node_key = `${prefix}${tokens.join("-")}`;
    const index = (input.index ?? tokens).map((f) => String(f).trim()).filter(Boolean);
    for (const f of index) {
        if (!declared.has(f))
            throw new GraphValidationError("UNKNOWN_ATTRIBUTE", type, `index field ${f} is not a declared attribute`, "index");
    }
    for (const [key, val] of [["title_key", input.title_key], ["description_key", input.description_key]]) {
        if (val !== undefined && !declared.has(val))
            throw new GraphValidationError("UNKNOWN_ATTRIBUTE", type, `${key} ${val} is not a declared attribute`, key);
    }
    const domain = String(input.domain ?? "entity").trim() || "entity";
    if (!IDENT.test(domain))
        throw new GraphValidationError("WRONG_TYPE", type, "domain must be a bare identifier", "domain");
    const flat = {
        type,
        parent,
        node_key,
 ...
 */
export async function createNodeSchema(bolt, resolver, input) {
    ...
    if (!parent)
        throw new GraphValidationError("UNKNOWN_TYPE", plan.type, `parent schema ${plan.parent} does not exist`, "parent");
    if (isStrutType(parent))
        throw new GraphValidationError("UNKNOWN_TYPE", plan.type, "Strut's own types cannot be extended (closed registry)", "parent");
    const result = await bolt.write(async (tx) => {
        if (existingType) {
            if (isStrutType(existingType))
                throw new GraphValidationError("UNKNOWN_TYPE", existingType, "Strut's own types are a closed in-code registry");
            ...
            if (live.length) {
                ...
                const have = new Set(live[0]["ks"]);
                const missing = {};
                for (const [k, v] of Object.entries(plan.attributes))
                    if (!have.has(k))
                        missing[k] = v;
                if (Object.keys(missing).length)
                    await tx.run(`MATCH (s:Schema {type: $t}) SET s += $missing`, { t: existingType, missing });
        ...
        await tx.run(`MATCH (child:Schema {type: $c}), (parent:Schema {type: $p})
       MERGE (child)-[r:CHILD_OF]->(parent) ON CREATE SET r.ref_id = $r`, { c: plan.type, p: parent, r: randomUUID() });
        return { created: true, ref_id, type: plan.type, added: [] };
    });
    if (result.created) {
        ...
        await schemaStatement(bolt, `CREATE FULLTEXT INDEX \`domain_${d}_attribute_index_v2\` IF NOT EXISTS FOR (n:\`Domain_${d}\`) ON EACH [${props}]
...
//# sourceMappingURL=schema-crud.js.map