...
import { int, txRows } from "./bolt.js";
...
export const RRF_WEIGHTS = { fulltext: 1.15, semantic: 1.0 };
...
export function rankHits(hits) {
    ...
    for (const h of hits) {
        ...
        if (!cur || h.raw_score > cur.raw_score)
            best.set(id, h);
    }
...
export function fuseHits(fulltext, semantic, buckets = {}) {
    const sources = [
        ["fulltext", rankHits(fulltext)],
        ["semantic", rankHits(semantic)],
        ...Object.entries(buckets).map(([k, v]) => [k, rankHits(v ?? [])]),
    ];
    const fused = new Map();
...
export function titleMatchMultiplier(qLower, valLower) {
    if (!valLower)
        return 1.0;
    if (valLower === qLower)
        return 4.0;
 ...
 *  `name`). */
export function applyTitleBoost(ranked, q, titleKeyFor) {
...
/** `_serialize_node`. */
export function serializeNode(n, extra) {
    const props = n.properties;
    const filtered = {};
    for (const [k, v] of Object.entries(props))
        if (!RESPONSE_STRIPPED_NODE_PROPERTIES.has(k))
            filtered[k] = v;
    if ("image_url" in filtered && typeof filtered["image_url"] !== "string")
        filtered["image_url"] = "";
    const out = {
    ...
    if (props["weight"] !== null && props["weight"] !== undefined)
        out.weight = Number(props["weight"]);
    if (extra)
        Object.assign(out, extra);
    return out;
}
function serializeEdge(type, props, source, target) {
    const filtered = {};
    for (const [k, v] of Object.entries(props))
        if (!RESPONSE_STRIPPED_EDGE_PROPERTIES.has(k))
            filtered[k] = v;
    const out = { source, target, ref_id: String(props["ref_id"] ?? ""), edge_type: type, properties: filtered };
    if (props["weight"] !== null && props["weight"] !== undefined)
        out.weight = Number(props["weight"]);
    return out;
}
function visibility(alias) {
    return [
...
export class GraphReader {
    ...
    async registerNamespace(name) {
        ...
        if (!ns)
            throw new GraphReadError("INVALID_INPUT", "namespace is required");
        return this.bolt.write(async (tx) => {
            ...
            const data = Array.isArray(existing[0]?.["data"]) ? existing[0]["data"] : [];
            if (data.includes(ns))
                return { namespace: ns, created: false };
            const ref_id = existing[0]?.["ref_id"] ?? randomUUID();
    ...
    async resolveNamespace(namespace) {
        ...
        if (ns.toLowerCase() === DEFAULT_NAMESPACE)
            return DEFAULT_NAMESPACE;
        const known = await this.listNamespaces();
        if (!known.includes(ns.toLowerCase()))
            throw new GraphReadError("INVALID_NAMESPACE", `namespace ${ns} is not registered`);
        return ns;
    }
    // ── nodes ───────────────────────────────────────────────────────────────
    /** GET /v2/nodes/:ref_id — `{name, node_type, ref_id, properties, weight?}`
     *  or null when absent/hidden. */
    async getNode(ref_id) {
        const rows = await this.bolt.run(`MATCH (n:Data_Bank {ref_id: $ref_id}) WHERE ${visibility("n")} RETURN n LIMIT 1`, { ref_id, blocked_statuses: BLOCKED_NODE_STATUSES });
        if (rows.length === 0)
            return null;
        const n = rows[0]["n"];
    ...
    async connectionCounts(ref_id, namespace) {
       ...
       RETURN type(r) AS edge_type, labels(m) AS m_labels, count(*) AS cnt`, { ref_id, namespace: ns, default_ns: DEFAULT_NAMESPACE, visible_labels: visible });
        const bucket = new Map();
        for (const r of rows) {
            const target = typeLabelOf(r["m_labels"]);
            if (!target)
                continue;
    ...
    async edgeCounts(ref_ids, namespace, pinNamespace = false, tx) {
        if (ref_ids.length === 0)
            return {};
        ...
        const rows = await this.rows(tx, `MATCH (n:Data_Bank)-[r]-(m)
       WHERE n.ref_id IN $ref_ids AND ${nsCond}
         ${visible.length ? "AND ANY(lbl IN labels(m) WHERE lbl IN $visible_labels)" : ""}
       RETURN n.ref_id AS ref_id, type(r) AS edge_type, count(*) AS cnt`, { ref_ids, namespace, default_ns: DEFAULT_NAMESPACE, visible_labels: visible });
        const out = {};
        for (const r of rows)
            (out[String(r["ref_id"])] ??= {})[String(r["edge_type"])] = Number(r["cnt"]);
        return out;
    }
    /**
     ...
     */
    async neighbors(ref_id, p = {}) {
        const where = [visibility("node")];
        const params = { ref_id, blocked_statuses: BLOCKED_NODE_STATUSES };
        ...
        if (limit)
            params["imp_limit"] = int(p.limit);
        const rows = await this.bolt.run(`MATCH (source:Data_Bank {ref_id: $ref_id})-[r]-(node)
       ...
       RETURN endNodes + [source] AS nodes, rels`, params);
        if (rows.length === 0)
            return { nodes: [], edges: [] };
        const nodes = [];
        const seen = new Set();
        for (const n of rows[0]["nodes"]) {
            const id = refIdOf(n);
            if (!id || seen.has(id))
                continue;
            seen.add(id);
    ...
    async search(p) {
        ...
        const vectorQueries = {};
        if (p.input_q?.trim())
            vectorQueries["input"] = p.input_q.trim();
        if (p.output_q?.trim())
            vectorQueries["output"] = p.output_q.trim();
        if (!q && Object.keys(vectorQueries).length === 0)
        ...
        const tx = undefined;
        {
            ...
            for (const d of domains)
                if (!allDomains.includes(d))
                    throw new GraphReadError("INVALID_DOMAIN", `unknown domain ${d}`);
            ...
            const base = { namespace, blocked_statuses: BLOCKED_NODE_STATUSES };
            if (types?.length) {
            ...
            const route = (suffix, global) => domains.length ? domains.map((d) => `domain_${d}${suffix}`) : existing.has(global) ? [global] : visibleDomains.map((d) => `domain_${d}${suffix}`);
            ...
            const layer = async (name, fn, empty) => {
                try {
                    return await fn();
                }
                catch (e) {
                    const msg = `${name} layer error: ${e?.message?.split("\n")[0] ?? String(e)}`;
                    warnings.push(msg);
                    console.warn(`[graph search] ${msg}`);
                    return empty;
                }
            };
            const runFulltext = async (indexes, lucene) => {
                const all = [];
                for (const index of indexes) {
                    const rows = await this.rows(tx, `CALL db.index.fulltext.queryNodes($index, $q_lucene, $fulltext_options) YIELD node AS n, score WHERE ${filter} RETURN n, score`, { ...base, index, q_lucene: lucene, fulltext_options: { limit: int(cap) } });
            ...
            if (q && ftIndexes.length) {
                fulltext = await layer("fulltext", async () => {
                    ...
                    if (hits.length === 0)
                        hits = await runFulltext(ftIndexes, `${q}~`);
                    return hits;
                }, []);
                if (fulltext.length >= cap)
                    ftTruncated = true;
            }
            const runVector = async (indexes, k, embedding) => {
                const all = [];
                for (const index of indexes) {
                    const rows = await this.rows(tx, `CALL db.index.vector.queryNodes($index, $k, $embedding) YIELD node AS n, score WHERE ${filter} RETURN n, score`, { ...base, index, k: int(k), embedding });
                    all.push(...rows.map(asHit));
                }
                return indexes.length > 1 ? rankHits(all) : all;
            };
                ...
                for (const [stem, text] of Object.entries(vectorQueries)) {
                    ...
                    buckets[stem] = await layer(`${stem}_q`, async () => {
                        ...
                        for (const lbl of labels) {
                            ...
                            if (!existing.has(idx))
                                continue; // migration may not have run yet
                            for (const h of await runVector([idx], VECTOR_Q_K, emb)) {
                                if (h.raw_score >= VECTOR_Q_SIM_FLOOR)
                                    all.push(h);
                            }
                        }
                        return rankHits(all);
            ...
            const titleKeyFor = (t) => (t ? titleKeys.get(t) ?? "name" : "name");
            for (const t of new Set(ranked.map((e) => typeLabelOf(e.node.labels)).filter((t) => !!t))) {
                const strut = getStrutSchema(t);
                if (strut)
                    titleKeys.set(t, strut.title_key);
                else {
                    const r = await this.rows(tx, `MATCH (s:Schema) WHERE toLower(s.type) = toLower($t) RETURN s.title_key AS k LIMIT 1`, { t });
                    if (typeof r[0]?.["k"] === "string")
                        titleKeys.set(t, r[0]["k"]);
                }
            }
    ...
    async vectorIndexedPairsLive(tx) {
        ...
        const out = [];
        for (const r of rows)
            for (const prop of r["v"] ?? [])
                out.push({ type: String(r["t"]), prop });
    ...
    async listSchemas(opts = {}) {
        const rows = await this.bolt.run(`MATCH (n:Schema) ${opts.includeDeleted ? "" : "WHERE (n.is_deleted IS NULL OR n.is_deleted = false)"}
       ...
       RETURN n, [node IN nodes(path) WHERE node <> n | node] AS ancestors, r, m`);
        ...
        for (const r of rows) {
            ...
            if (!byRef.has(ref)) {
                const merged = { ...n.properties };
                for (const a of r["ancestors"] ?? []) {
                    for (const [k, v] of Object.entries(a.properties)) {
                        if (k in merged || SCHEMA_PARENT_PROPERTIES_TO_IGNORE.has(k))
                            continue;
                        merged[k] = v;
                    }
                }
    ...
    async getSchema(type) {
        const rows = await this.bolt.run(`MATCH (n:Schema) WHERE ${type === "Thing" ? "n.type = $t" : "toLower(n.type) = toLower($t)"}
       OPTIONAL MATCH path = (n)-[:CHILD_OF*1..]->(ancestor:Schema)
       RETURN n, [node IN nodes(path) WHERE node <> n | node] AS ancestors LIMIT 1`, { t: type });
        if (rows.length === 0)
            return null;
        const n = rows[0]["n"];
        const merged = { ...n.properties };
        for (const a of rows[0]["ancestors"] ?? []) {
            for (const [k, v] of Object.entries(a.properties)) {
                if (k in merged || SCHEMA_PARENT_PROPERTIES_TO_IGNORE.has(k))
                    continue;
                merged[k] = v;
        ...
        const inherited = {};
        for (const [k, v] of Object.entries(split.attributes))
            if (!own.has(k))
                inherited[k] = v;
    ...
    async canonicalTypes(raw) {
        if (!this.opts.resolver)
            return raw;
        const out = [];
        for (const t of raw)
            out.push((await this.opts.resolver.resolveType(t)) ?? t);
        return out;
    }
    rows(tx, cypher, params = {}) {
        return tx ? txRows(tx, cypher, params) : this.bolt.run(cypher, params);
    }
}
/** `_split_schema_properties`. */
export function splitSchema(merged) {
    const core = {};
    const attributes = {};
    for (const [k, v] of Object.entries(merged))
        (SCHEMA_CORE_PROPERTIES.has(k) ? core : attributes)[k] = v;
...
export function inheritedAttributes(schemas) {
    const byType = new Map(schemas.map((s) => [s.type, s]));
    return schemas.map((s) => {
        const parent = typeof s["parent"] === "string" ? byType.get(s["parent"]) : undefined;
        if (!parent)
            return { ...s, inherited_attributes: {} };
