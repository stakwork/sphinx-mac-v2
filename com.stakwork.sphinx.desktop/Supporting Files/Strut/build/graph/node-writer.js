...
import { SchemaResolver, fromStrut } from "./schema-resolver.js";
...
export class GraphValidationError extends Error {
    ...
    type;
    attribute;
    constructor(code, type, message, attribute) {
...
const isOptionalType = (t) => t.startsWith("?");
const baseOf = (t) => (isOptionalType(t) ? t.slice(1) : t);
/**
 ...
 */
export function validateNode(typeOrSchema, data) {
    let schema;
    if (typeof typeOrSchema === "string") {
        const strut = getStrutSchema(typeOrSchema);
        if (!strut)
            throw new GraphValidationError("UNKNOWN_TYPE", typeOrSchema, "not a registered Strut type (resolve other types via SchemaResolver)");
    ...
    for (const [name, t] of Object.entries(attrs)) {
        ...
        if (!present) {
            ...
            if (!isOptionalType(t))
                throw new GraphValidationError("MISSING_REQUIRED", type, `required ${base} attribute is missing`, name);
            continue;
        }
    ...
    for (const field of schema.node_key.split("-").slice(1)) {
        ...
        if (!hit)
            throw new GraphValidationError("MISSING_REQUIRED", type, "attribute referenced in node_key is missing", field);
...
function normalizeValue(type, name, base, raw) {
    switch (base) {
        case "string":
            if (typeof raw !== "string")
                throw wrong(type, name, "string", raw);
            return { value: raw, param: raw };
        case "boolean":
            if (typeof raw !== "boolean")
                throw wrong(type, name, "boolean", raw);
            return { value: raw, param: raw };
        case "int":
            // Booleans are not ints (Python quirk we do not replicate).
            if (typeof raw !== "number" || !Number.isInteger(raw))
                throw wrong(type, name, "int", raw);
            return { value: raw, param: int(raw) };
        case "float":
            // int-where-float is accepted (as in jarvis) and written as a FLOAT.
            if (typeof raw !== "number" || !Number.isFinite(raw))
                throw wrong(type, name, "float", raw);
            return { value: raw, param: raw };
        case "datetime": {
            const secs = toEpochSeconds(raw);
            if (secs === null)
                throw new GraphValidationError("INVALID_DATETIME", type, "expected ISO-8601 string or epoch number", name);
            return { value: secs, param: int(secs) };
        }
        case "list": {
            if (!Array.isArray(raw))
                throw wrong(type, name, "list", raw);
...
function wrong(type, name, expected, raw) {
...
export function toEpochSeconds(v) {
    ...
    if (v instanceof Date && !Number.isNaN(v.getTime()))
        return Math.trunc(v.getTime() / 1000);
    return null;
}
// ── node_key (§1) ───────────────────────────────────────────────────────────
...
export function sanitizeKeyValue(v) {
    return String(v)
        .trim()
        .replace(/ /g, "")
        .toLowerCase()
 ...
 */
export function composeNodeKey(schema, values) {
    ...
    tokens.forEach((tok, i) => {
        ...
        if (!lower.has(tok.toLowerCase()))
            throw new GraphValidationError("MISSING_REQUIRED", schema.type, "node_key property missing", tok);
        parts.push(sanitizeKeyValue(lower.get(tok.toLowerCase())));
    });
    const composed = parts.join("-");
    if (composed.length <= MAX_NODE_KEY_LENGTH || parts.length < 2)
        return composed;
...
const nonBlank = (v) => v !== null && v !== undefined && String(v).trim().length > 0;
/**
 ...
 */
export function searchFieldsFor(schema, values) {
    ...
    for (const [f, v] of Object.entries(values)) {
        if (DATA_BANK_EXCLUDED_FIELDS.has(f) || seen.has(f) || !nonBlank(v))
            continue;
        out.push(f);
        seen.add(f);
    }
...
export function buildSearchText(schema, values) {
...
export function renderVectorField(prop, text) {
    const t = text.trim();
    if (!t)
        return null;
...
export class NodeWriter {
    ...
    async prepareMany(inputs, opts = {}) {
        ...
        const prepared = validated.map((v, idx) => {
            ...
            for (const vi of v.schema.vector_index) {
                ...
                if (rendered)
                    jobs.push({ idx, column: embeddingColumn(vi), text: rendered });
            }
            const onMatch = {};
    ...
    /** Write one node. */
    async write(input, mode = "create", opts = {}) {
     ...
     */
    async writeMany(inputs, mode = "create", opts = {}) {
        if (inputs.length === 0)
            return [];
        const namespace = opts.namespace ?? this.bolt.namespace;
    ...
    async update(ref_id, patch) {
        ...
        return this.bolt.write(async (tx) => {
            ...
            if (rows.length === 0)
                throw new GraphValidationError("NOT_FOUND", "?", `no node with ref_id ${ref_id}`);
            ...
            if (!schema)
                throw new GraphValidationError("UNKNOWN_TYPE", type, "stored node's type has no schema");
            const attrs = schema.attributes;
            const current = {};
            ...
            for (const k of remove) {
                const t = attrs[k];
                if (!t)
                    throw new GraphValidationError("UNKNOWN_ATTRIBUTE", type, "attribute is not declared on the schema", k);
                if (!isOptionalType(t))
                    throw new GraphValidationError("MISSING_REQUIRED", type, "required attribute cannot be removed", k);
                delete current[k];
            }
            ...
            if (rekeyed) {
                const clash = await txRows(tx, `MATCH (m:\`${schema.type}\` {node_key: $k, namespace: $ns}) WHERE m.ref_id <> $ref_id RETURN m.ref_id AS ref_id LIMIT 1`, { k: node_key, ns: props["namespace"], ref_id });
                if (clash.length)
                    throw new GraphValidationError("DUPLICATE_KEY", type, `Node already exists in the graph with node_key: ${node_key}`);
            }
            // Everything the fresh payload would carry, minus identity stamps.
            const setMap = { ...p.onMatch, node_key };
            const drop = new Set(remove);
            if (!("Data_Bank" in setMap))
                for (const k of ["Data_Bank", "_search_fields_used", "text_embeddings"])
                    drop.add(k);
            else if (!this.opts.embedder && setMap["Data_Bank"] !== props["Data_Bank"])
                drop.add("text_embeddings");
            for (const vi of schema.vector_index) {
                const col = embeddingColumn(vi);
                if (!(vi in merged))
                    drop.add(col);
                else if (!this.opts.embedder && merged[vi] !== props[vi])
                    drop.add(col);
            }
            for (const k of drop)
                delete setMap[k];
            ...
            for (const k of Object.keys(attrs))
                if (k in props && !(k in setMap) && !drop.has(k) && !(k in merged))
                    drop.add(k);
    ...
    async softDelete(ref_id) {
        const rows = await this.bolt.run(`MATCH (n:\`${STRUT_DOMAIN_LABEL}\` {ref_id: $ref_id}) SET n.is_deleted = true RETURN n.ref_id AS ref_id`, { ref_id });
 ...
 */
async function mergeBatch(tx, schema, namespace, nodes, mode) {
    const type = schema.type;
    const labels = [`\`${type}\``, "Node", "Data_Bank", ...schema.domainLabels.map((l) => `\`${l}\``)].join(":");
    const before = await txRows(tx, `UNWIND $keys AS k
     MATCH (n:\`${type}\` {node_key: k, namespace: $ns})
     RETURN k AS node_key, coalesce(n.is_deleted, false) AS deleted, coalesce(n.is_muted, false) AS muted`, { keys: nodes.map((n) => n.node_key), ns: namespace });
    const state = new Map(before.map((r) => [r["node_key"], { deleted: r["deleted"], muted: r["muted"] }]));
    const rows = await txRows(tx, `UNWIND $rows AS row
    ...
    const byKey = new Map(rows.map((r) => [r["node_key"], r]));
    return nodes.map((n) => {
        ...
        if (created)
            outcome = "created";
        else if (prior?.deleted && !prior.muted)
            outcome = "restored";
        else if (mode === "upsert")
            outcome = "updated";
        else
            outcome = "existing";
        return { ref_id: r["ref_id"], node_key: n.node_key, outcome };
    });
}
