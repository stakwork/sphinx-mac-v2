...
import { SCHEMA_CORE_PROPERTIES, STRUT_DOMAIN, STRUT_SCHEMAS, effectiveAttributes, getStrutSchema, } from "./strut-schemas.js";
...
export class SchemaResolver {
     ...
     */
    async resolveType(raw, tx) {
        const key = raw.trim();
        if (!key)
            return null;
        if (getStrutSchema(key))
            return key;
        const c = this.types.get(key.toLowerCase());
        if (this.fresh(c))
            return c.value;
        let out = null;
        const s = await this.rows(tx, `MATCH (n:Schema) WHERE toLower(n.type) = toLower($t) AND (n.is_deleted IS NULL OR n.is_deleted = false) AND n.type <> "*"
       RETURN n.type AS t ORDER BY CASE WHEN n.type = $t THEN 0 ELSE 1 END, n.type LIMIT 1`, { t: key });
        if (s.length)
            out = String(s[0]["t"]);
        else {
            const labels = await this.rows(tx, `CALL db.labels() YIELD label WHERE toLower(label) = toLower($t)
         RETURN label ORDER BY CASE WHEN label = $t THEN 0 ELSE 1 END, label LIMIT 1`, { t: key });
            if (labels.length)
                out = String(labels[0]["label"]);
        }
        if (out && getStrutSchema(out))
            out = getStrutSchema(out).type;
    ...
    async schema(raw, tx) {
        ...
        if (strut)
            out = fromStrut(strut, hidden);
        else
            out = await this.fromDb(type, hidden, tx);
        this.schemas.set(type, { at: Date.now(), value: out });
        return out;
    }
    async fromDb(type, hidden, tx) {
        const rows = await this.rows(tx, `MATCH (n:Schema {type: $t})
       ...
       ORDER BY size(chain) DESC LIMIT 1`, { t: type });
        if (rows.length === 0)
            return null;
        const own = rows[0]["n"].properties;
        const chain = (rows[0]["chain"] ?? []).filter(Boolean);
        const merged = { ...own };
        for (const a of rows[0]["ancestors"] ?? []) {
            for (const [k, v] of Object.entries(a)) {
                if (k in merged || k === "type_description")
                    continue;
                merged[k] = v;
            }
        }
        ...
        return {
            type,
            parent: typeof merged["parent"] === "string" ? merged["parent"] : undefined,
            ...
            attributes,
            title_key: typeof merged["title_key"] === "string" ? merged["title_key"] : undefined,
            description_key: typeof merged["description_key"] === "string" ? merged["description_key"] : undefined,
            domainLabels,
            isStrut: false,
        };
    }
     ...
     */
    async edgeSchema(edge, sourceType, targetType, tx) {
        if (EDGE_TYPES_ALLOWLIST.has(edge))
            return { source: sourceType, target: targetType, edge_type: edge, properties: {}, via: "allowlist" };
        const key = `${sourceType}|${edge}|${targetType}`;
        const c = this.edges.get(key);
        if (this.fresh(c))
            return c.value;
        const p = { s: sourceType, t: targetType, e: edge };
        const pick = (rows, via) => rows.length
            ? { source: String(rows[0]["source"]), target: String(rows[0]["target"]), edge_type: String(rows[0]["edge_type"]), properties: rows[0]["props"], via }
            : null;
        let match = pick(await this.rows(tx, `MATCH (source:Schema)-[r]->(target:Schema)
           WHERE toLower(source.type) = toLower($s) AND toLower(target.type) = toLower($t) AND toLower(type(r)) = toLower($e)
           RETURN source.type AS source, target.type AS target, type(r) AS edge_type, properties(r) AS props LIMIT 1`, p), "exact") ??
            pick(await this.rows(tx, `MATCH path = (child:Schema)-[:CHILD_OF*1..${MAX_ANCESTOR_WALK_DEPTH}]->(ancestor:Schema)
           ...
           ORDER BY length(path) ASC LIMIT 1`, p), "ancestor") ??
            pick(await this.rows(tx, `MATCH path = (child:Schema)-[:CHILD_OF*1..${MAX_ANCESTOR_WALK_DEPTH}]->(ancestor:Schema)
           ...
           ORDER BY length(path) ASC LIMIT 1`, p), "ancestor") ??
            pick(await this.rows(tx, `MATCH src_path = (src_child:Schema)-[:CHILD_OF*1..${MAX_ANCESTOR_WALK_DEPTH}]->(src_anc:Schema)
           ...
           ORDER BY (length(src_path) + length(tgt_path)) ASC LIMIT 1`, p), "ancestor") ??
            pick(await this.rows(tx, `MATCH (source:Schema {type: "*"})-[r]->(target:Schema {type: "*"})
           WHERE toLower(type(r)) = toLower($e)
           RETURN source.type AS source, target.type AS target, type(r) AS edge_type, properties(r) AS props LIMIT 1`, p), "wildcard");
        if (match && match.via === "exact")
            match = { ...match, source: sourceType, target: targetType };
     ...
     */
    async createEdgeSchema(sourceType, edge, targetType) {
        const edgeType = edge.toUpperCase().replace(/ /g, "_");
        if (!/^[A-Z][A-Z0-9_]*$/.test(edgeType))
        ...
        return this.bolt.write(async (tx) => {
            for (const t of [sourceType, targetType]) {
                ...
                if (exists.length === 0)
                    throw new Error(`schema ${t} does not exist`);
            }
            const ref_id = randomUUID();
            const rows = await txRows(tx, `MATCH (source:Schema {type: $s}), (target:Schema {type: $t})
         MERGE (source)-[r:\`${edgeType}\`]->(target)
         ON CREATE SET r.ref_id = $ref_id
         RETURN r.ref_id = $ref_id AS created, r.ref_id AS ref_id`, { s: sourceType, t: targetType, ref_id });
            this.edges.clear();
...
export function fromStrut(strut, hidden) {
...
export function strutNodeSchemas() {
