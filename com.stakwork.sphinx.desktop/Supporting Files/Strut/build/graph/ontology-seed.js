 ...
 */
export function searchableAttributesOf(schema) {
    const out = new Set();
    ...
    for (const [k, v] of Object.entries(schema)) {
        if (SCHEMA_CORE_PROPERTIES.has(k))
            continue;
...
export async function seedJarvisOntology(bolt, fixture = JARVIS_ONTOLOGY) {
    const report = { createdSchemas: [], createdEdgeSchemas: 0, domains: [], skippedSchemaObjects: [] };
    ...
    for (const schema of fixture.schemas) {
        ...
        if (hit.length)
            continue;
        const props = { ...schema };
        if (typeof props["ref_id"] !== "string")
            props["ref_id"] = randomUUID();
    ...
    for (const schema of fixture.schemas) {
        ...
        if (typeof parent !== "string" || !parent)
            continue;
        await bolt.run(`MATCH (child:Schema {type: $c}), (parent:Schema {type: $p})
       MERGE (child)-[r:CHILD_OF]->(parent) ON CREATE SET r.ref_id = $ref_id`, { c: schema["type"], p: parent, ref_id: randomUUID() });
    }
    // 3. Per-type constraint + index for every schema with a node_key.
    for (const schema of fixture.schemas) {
        const type = String(schema["type"]);
        if (type === "*" || typeof schema["node_key"] !== "string")
            continue;
        await ddl(`CREATE CONSTRAINT ${`unique_${type.toLowerCase()}_node_key`} IF NOT EXISTS FOR (n:\`${type}\`) REQUIRE (n.node_key, n.namespace) IS UNIQUE`);
    ...
    for (const e of fixture.edge_schemas) {
        ...
        const rows = await bolt.run(`MATCH (s:Schema {type: $s}), (t:Schema {type: $t})
       MERGE (s)-[r:\`${e.edge}\`]->(t)
       ON CREATE SET r = $props, r.__created = true
       WITH r, coalesce(r.__created, false) AS created
       REMOVE r.__created
       RETURN created`, { s: e.source, t: e.target, props });
        if (rows[0]?.["created"])
            report.createdEdgeSchemas++;
    }
    ...
    for (const s of fixture.schemas) {
        if (s["type"] === "*")
            continue;
        for (const a of searchableAttributesOf(s))
            attrs.add(a);
        if (typeof s["domain"] === "string" && s["domain"])
            domains.add(s["domain"].toLowerCase());
    }
    attrs.add("node_key");
    const props = [...attrs].sort().map((p) => `n.\`${p}\``).join(", ");
    await ddl(`CREATE FULLTEXT INDEX data_bank_attribute_index_v2 IF NOT EXISTS FOR (n:Data_Bank) ON EACH [${props}]
    ...
    for (const d of [...domains].sort()) {
        await ddl(`CREATE FULLTEXT INDEX \`domain_${d}_attribute_index_v2\` IF NOT EXISTS FOR (n:\`Domain_${d}\`) ON EACH [${props}]
       OPTIONS { indexConfig: { \`fulltext.analyzer\`: 'english' } }`);
    ...
    for (const s of fixture.schemas) {
        ...
        for (const prop of vi) {
            ...
            await ddl(`CREATE VECTOR INDEX \`${String(s["type"]).toLowerCase()}_${stem}_vector_index\` IF NOT EXISTS FOR (n:\`${s["type"]}\`) ON n.${stem}_embeddings ${VECTOR_OPTIONS}`);
        }
    }
    report.domains = [...domains].sort();
    return report;
}
//# sourceMappingURL=ontology-seed.js.map