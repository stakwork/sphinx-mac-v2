 ...
 */
const STEP_SRC = (type, desc) => `import { z } from "zod";
...
`;
export function workspaceConformance(impl) {
    describe(`WorkspaceStore conformance: ${impl.name}`, { skip: impl.skip ?? false }, () => {
        ...
        it("workflow publish → list → metadata → source → hash round-trip", async () => {
            ...
            assert.deepEqual(list.map((w) => [w.name, w.activeVersion, w.versions, w.description, w.category, w.publisher]), [["wf", "v1", ["v1"], "first", "exp", "me"]]);
        ...
        it("versions, active switching, content dedup, category, params", async () => {
            ...
            const second = await ws.publishWorkflowByContent("wf", (await ws.getWorkflowSource("wf", "v1")).replace("old", "new"));
        ...
        it("reactivateKnown: false keeps a workspace edit active across a reseed", async () => {
            ...
            const updated = await ws.publishWorkflowByContent("wf", seed.replace("old", "newer"), undefined, undefined, undefined, { reactivateKnown: false });
        ...
        it("publishStep reactivateKnown: false keeps a workspace edit active across a reseed", async () => {
            ...
            assert.ok((await ws.getStepSource("kept"))?.code.includes("two"), "the materialized (loadable) source is still the edit");
        ...
        it("step publish → list → versions → source → active switching → delete", async () => {
            ...
            assert.deepEqual((await ws.listSteps()).map((s) => [s.type, s.description, s.publisher]), [["my-step", "two", "svc"]]);
...
//# sourceMappingURL=workspace-conformance.js.map