...
export default defineStep({
    type: "meta/get-run",
    description: "Get one run of an agent-authored workflow: its summary (input, output, status, error, duration) and event log. Events are slimmed by default (no payloads) to stay token-cheap; set fullEvents:true for each step's input/output. Use to debug why a candidate run failed. Runs of workflows the agent surface did not author are refused.",
    input: z.object({
