...
function nameOf(nodeType, p) {
    const strut = nodeType ? getStrutSchema(nodeType) : undefined;
    if (strut && typeof p[strut.title_key] === "string")
        return p[strut.title_key];
    return p.name ?? p.workflow_name ?? p.episode_title ?? p.entity;
}
function descriptionOf(nodeType, p) {
    const strut = nodeType ? getStrutSchema(nodeType) : undefined;
    if (strut && typeof p[strut.description_key] === "string" && strut.description_key !== strut.title_key)
        return p[strut.description_key];
    return p.description ?? p.summary ?? p.text ?? "";
}
export default defineStep({
    type: "graph/graph-search",
    description: "Search the strut knowledge graph for nodes — workflows, workflow versions, steps, runs, agent sessions, tool calls, chats, turns, and any jarvis-owned types sharing the database. " +
    ...
    input: z.object({
        ...
        input_q: z
            .string()
            .optional()
            .describe("Semantic search scoped to node INPUT schemas — find nodes by what they take as input, " +
            "e.g. 'a video file url'. Applies to node types with input embeddings (StrutWorkflowVersion, StrutStep)."),
        output_q: z
            .string()
            .optional()
            .describe("Semantic search scoped to node OUTPUT schemas — find nodes by what they produce, " +
