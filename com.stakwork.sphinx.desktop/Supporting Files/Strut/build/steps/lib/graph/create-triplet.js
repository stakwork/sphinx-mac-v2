...
export default defineStep({
    ...
    description: "Assert a fact into the strut knowledge graph as DATA: a triplet of source node -[edge]-> target node " +
    ...
    input: z.object({
        ...
        create_schema_if_missing: z
            ...
            .optional()
            .default(false)
            .describe("Auto-create the edge schema when the (source_type, edge_type, target_type) relationship is not yet in the ontology. " +
    ...
    async run(cfg, ctx) {
        for (const err of [
            ...
            if (err)
                return `graph/create-triplet invalid input — ${err}`;
        }
        try {
            ...
            try {
                ...
                return (`graph/create-triplet: nodes resolved (source=${sourceRef}, target=${targetRef}) ` +
...
//# sourceMappingURL=create-triplet.js.map