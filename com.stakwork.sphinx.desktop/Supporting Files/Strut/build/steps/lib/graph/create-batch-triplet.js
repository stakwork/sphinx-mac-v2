...
function validateTripletSide(side, refId, nodeType, nodeData) {
...
function nodeDedupKey(nodeType, nodeData) {
...
export default defineStep({
    ...
    description: "Assert MANY facts into the strut knowledge graph in a single call. " +
    ...
    async run(cfg, ctx) {
        ...
        const resolveSide = async (side, refId, nodeType, nodeData) => {
            if (refId)
                return refId;
        ...
        for (const t of cfg.triplets) {
            const invalid = validateTripletSide("source", t.source_ref_id, t.source_type, t.source_data) ??
        ...
        return withAccessedNodes({ requested: cfg.triplets.length, succeeded: results.length - failed, failed, results }, 
        // Provenance: both endpoints of every edge that was written.
        results.filter((r) => !r.error).flatMap((r) => [{ ref_id: r.source_ref_id }, { ref_id: r.target_ref_id }]));
    },
});
//# sourceMappingURL=create-batch-triplet.js.map