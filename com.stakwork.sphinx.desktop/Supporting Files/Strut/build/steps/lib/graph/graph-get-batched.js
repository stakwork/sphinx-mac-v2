...
function collapseConnectionCounts(counts) {
    const out = {};
...
export default defineStep({
    ...
    description: `Resolve up to ${BATCH_MAX} nodes in one call by ref_id — the batched form of graph_graph_get. ` +
    ...
    async run(cfg, ctx) {
        ...
        async function fetchNode(ref_id) {
            try {
                const raw = await b.reader.getNode(ref_id);
                if (!raw)
                    return { ref_id, error: `node not found: ${ref_id}` };
        ...
        nodes.filter((n) => !n["error"]).map((n) => ({ ref_id: n["ref_id"], node_type: n["node_type"] })));
    },
});
//# sourceMappingURL=graph-get-batched.js.map