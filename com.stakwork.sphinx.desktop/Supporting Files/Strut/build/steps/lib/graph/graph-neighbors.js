...
function deriveNodeName(node, p) {
    const candidates = [
...
export default defineStep({
    ...
    description: "Return all nodes adjacent (one hop) to a node in the strut knowledge graph, " +
    ...
    async run(cfg, ctx) {
        try {
            ...
            for (const node of data.nodes ?? []) {
                if (node.ref_id !== cfg.ref_id) {
                    nodeMap.set(node.ref_id, {
                        ...
                        name: deriveNodeName(node, (node.properties ?? {})),
                        edges: (node.edges ?? {}),
                    });
                }
            }
            ...
            for (const edge of data.edges ?? []) {
                ...
                if (neighborRefId === cfg.ref_id)
                    continue; // self-loop guard
                if (seen.has(neighborRefId))
                    continue; // parallel-edge dedup: keep the first
                seen.add(neighborRefId);
                const detail = nodeMap.get(neighborRefId);
                const importance = edge.properties?.["importance"];
                neighbors.push({
