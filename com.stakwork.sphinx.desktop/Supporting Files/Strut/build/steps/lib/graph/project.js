...
export default defineStep({
    type: "graph/project",
    description: "Project strut's own run and chat history into the knowledge graph (StrutRun / StrutAgentSession / " +
    ...
    async run(cfg, ctx) {
        try {
            ...
            const [{ FileRunStore }, { FileChatStore }, { FileWorkspaceStore }, { Neo4jWorkspaceStore }, { projectAll }, { graphWorkspaceRequested }] = await Promise.all([
                import("../../../store.js"),
            ...
            const report = await projectAll(b, { store: new FileRunStore(dataDir), chatStore: cfg.chats === false ? undefined : new FileChatStore(dataDir), workflows }, { limitPerWorkflow: cfg.limitPerWorkflow, skipSettled: cfg.skipSettled });
