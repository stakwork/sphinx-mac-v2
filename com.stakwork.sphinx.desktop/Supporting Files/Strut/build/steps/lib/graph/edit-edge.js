import { z } from "zod";
import { defineStep, withAccessedNodes } from "../../../core.js";
...
export default defineStep({
    type: "graph/edit-edge",
    description: "Update the properties of an EXISTING edge in the strut knowledge graph (writes live to the graph). " +
    ...
    async run(cfg, ctx) {
        ...
        if (!byRef && !byTriple)
            return "graph/edit-edge invalid input — pass edge_ref_id, or source_ref_id + edge_type + target_ref_id";
        if (byRef && byTriple)
            return "graph/edit-edge invalid input — pass EITHER edge_ref_id OR the source_ref_id/edge_type/target_ref_id triple, not both";
        ...
        if (!hasSet && !hasDelete)
            return "graph/edit-edge invalid input — pass at least one change: edge_data (properties to set) or properties_to_be_deleted";
        try {
            const b = await graphCtx(ctx);
            const r = await b.edges.update(byRef ? { ref_id: cfg.edge_ref_id } : { edge: cfg.edge_type, source_ref_id: cfg.source_ref_id, target_ref_id: cfg.target_ref_id }, { set: cfg.edge_data ?? {}, remove: cfg.properties_to_be_deleted ?? [] });
...
//# sourceMappingURL=edit-edge.js.map