...
export default defineStep({
    ...
    async run(cfg, ctx) {
        ...
        const newestModifiedTime = files.reduce((max, f) => {
            if (!f.modifiedTime)
                return max;
...
export function buildQuery(cfg) {
    const clauses = [];
    if (cfg.folderId)
        clauses.push(`'${cfg.folderId}' in parents`);
    if (cfg.modifiedAfter)
        clauses.push(`modifiedTime > '${cfg.modifiedAfter}'`);
    if (cfg.mimeType)
        clauses.push(`mimeType = '${cfg.mimeType}'`);
    if (!cfg.includeTrashed)
