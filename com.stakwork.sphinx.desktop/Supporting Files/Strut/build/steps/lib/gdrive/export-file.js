...
export default defineStep({
    ...
    async run(cfg, ctx) {
        ...
        let raw;
        if (isNative) {
            const exportMime = cfg.exportMimeType ?? EXPORT_MIME[mimeType] ?? DEFAULT_NATIVE_EXPORT;
            ({ data: raw } = await client.files
                .export({ fileId: cfg.fileId, mimeType: exportMime })
                .catch((err) => {
                if (statusOf(err) === 400) {
                    throw new Error(`Google Drive cannot export "${meta.name ?? cfg.fileId}" (${mimeType}) as "${exportMime}". Set a supported \`exportMimeType\` for this file type.`);
                }
                throw describeDriveError(err, resource, haveAuth);
            }));
        }
