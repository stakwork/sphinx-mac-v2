...
export async function slackToken(explicit, ctx) {
    const token = explicit ?? (await ctx?.services?.secrets?.get("SLACK_BOT_TOKEN"));
    if (!token) {
        throw new Error("No Slack token. Pass `token` in config or add a SLACK_BOT_TOKEN secret (a bot token, starts with xoxb-).");
    }
    return token;
}
...
export async function slackCall(ctx, method, token, opts = {}) {
    const http = ctx?.services?.http;
    if (!http)
        throw new Error("slack steps require ctx.services.http");
...
export function describeSlackError(error, method) {
    const hints = {
        channel_not_found: "channel not found — use the channel ID (e.g. C0123ABCD) and make sure the bot has been added to it",
        not_in_channel: "the bot isn't a member of that channel — /invite it, or post to a channel it has joined",
        ...
        invalid_auth: "invalid Slack token — check SLACK_BOT_TOKEN (a bot token starts with xoxb-)",
