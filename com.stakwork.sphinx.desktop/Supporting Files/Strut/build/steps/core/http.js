...
import { httpCapability } from "../../capabilities.js";
...
export default defineStep({
    type: "http",
    description: `Make an HTTP request. Output: { status, body } (body is parsed JSON when the response is JSON, else text). Throws on a non-2xx response.\n\n` +
    ...
    async run(cfg, ctx) {
        ...
        if (!res.ok) {
            throw new Error(`HTTP ${cfg.method} ${cfg.url} returned ${res.status}: ${typeof res.body === "string" ? res.body : JSON.stringify(res.body)}`);
        }
