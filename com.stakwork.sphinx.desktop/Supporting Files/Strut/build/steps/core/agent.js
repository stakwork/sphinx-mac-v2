import { z } from "zod";
import { accessedNodesOf, defineStep, withAccessedNodes } from "../../core.js";
 ...
 *  collapsed (name only). */
function renderTreeAtDepth(root, maxDepth) {
    const lines = [];
    const walk = (node, level) => {
        for (const name of [...node.dirs.keys()].sort()) {
            lines.push(`${"  ".repeat(level - 1)}${name}/`);
            if (NOISE_DIRS.has(name))
                continue; // collapse noise dirs
            if (level < maxDepth)
                walk(node.dirs.get(name), level + 1);
        }
 ...
 */
export function repoTree(files, opts = {}) {
    ...
    for (let d = 2; d <= maxDepth; d++) {
        ...
        if (rendered.split("\n").length > maxLines)
            break; // too many → keep previous depth
        best = rendered;
        depth = d;
    }
...
async function stakgraphSummary(filePath, cwd) {
...
async function fulltextSearch(query, cwd) {
    if (!existsSync(cwd))
 ...
 */
export function textEdit(input, roots) {
    let target;
    try {
 ...
 */
export function expandAgentTools(names, registry) {
    const out = [];
    const seen = new Set();
 ...
 *                  turn is possible.
 */
export function classifyFinalAnswerStop(finalFound, stepsUsed, maxSteps) {
    if (finalFound)
        return "done";
...
export function degenerateSchemaFields(schema, output) {
    const s = schema;
    if (!s || typeof s !== "object" || !Array.isArray(s.required))
        return [];
    const props = s.properties ?? {};
    const obj = (output && typeof output === "object" ? output : {});
    const bad = [];
    for (const key of s.required) {
        if (typeof key !== "string" || props[key]?.type !== "string")
            continue;
        const v = obj[key];
        if (typeof v !== "string" || !v.trim() || FILLER_VALUE.test(v.trim()))
            bad.push(key);
    }
    return bad;
}
...
export function isTransientStreamError(err) {
    ...
    for (let e = err; e && !seen.has(e); e = e.cause) {
        ...
        if (e.name === "AbortError" || msg.includes("abort"))
            return false;
        if (msg === "terminated" || // undici: response body stream died mid-flight
...
const STREAM_ERROR_NUDGE = "Your previous message was interrupted mid-stream by a transient connection error; " +
 ...
 */
export function buildRegistryTools(names, registry, ctx, 
        ...
        out[toolNameFor(stepType)] = toolFactory({
            ...
            execute: async (input, options) => {
                ...
                try {
                    ...
                    return `Error: invalid input for "${stepType}": ${e.message}`;
                }
                ...
                const base = ctx ??
                    ({ runId: "", path: "", scope: {}, input: undefined, emit: (async () => { }), services: undefined });
                const childCtx = options?.strutToolPath
...
export function maskSecretValues(text, values) {
    let out = text;
    for (const v of values)
        out = out.split(v).join("[MASKED_SECRET]");
    return out;
}
...
export function maskDeep(value, values) {
    if (!values.length)
        return value;
    if (typeof value === "string")
        return maskSecretValues(value, values);
    ...
    if (Array.isArray(value))
        return withAccessedNodes(value.map((x) => maskDeep(x, values)), accessedNodesOf(value) ?? []);
    if (value && typeof value === "object") {
        const out = {};
        for (const [k, v] of Object.entries(value))
            out[k] = maskDeep(v, values);
 ...
 */
export function wrapToolsWithMask(tools, secretValues) {
    ...
    for (const t of Object.values(tools)) {
        ...
        if (typeof orig !== "function")
            continue;
        t.execute = async (input, opts) => maskDeep(await orig(input, opts), secretValues);
    }
}
export function wrapToolsWithEmit(tools, ctx) {
    ...
    for (const [name, t] of Object.entries(tools)) {
        ...
        t.execute = async (input, opts) => {
            ...
            try {
                ...
                const optsWithPath = typeof opts === "object" && opts !== null
                    ? { ...opts, strutToolPath: path }
                    : { strutToolPath: path };
                const out = await orig(input, optsWithPath);
                ...
                await emit({ type: "step.error", path, stepType: `tool:${name}`, error: { message: e.message } });
                throw e;
            }
        };
    }
...
export default defineStep({
    ...
    input: z.object({
        ...
        returnMessages: z
            .boolean()
            .default(false)
            .describe("include the FULL tool-loop session (`messages`) in the output. Off by default — the session is huge and the runner persists every step's output, so returning it bloats events.jsonl/run.json and buries `result`. Turn on only for a fork/sub-agent that needs the transcript."),
    }),
    output: z.any(),
    async run(cfg, ctx) {
        ...
        const provider = cfg.provider ?? process.env["STRUT_LLM_PROVIDER"] ?? getProviderForModel(modelName);
        if (!PROVIDERS.includes(provider)) {
            throw new Error(`Unknown LLM provider: "${provider}". Supported: ${PROVIDERS.join(", ")}`);
        }
        ...
        if (provider === "anthropic") {
            ...
            textEditorTool = anthropic.tools.textEditor_20250728({
                execute: async (input) => textEdit(input, [cfg.cwd, os.tmpdir()]),
            });
        ...
        const allTools = {
            repo_overview: tool({
                description: "Get a high-level map of the codebase under the working dir (tracked files across any sub-repos). Use it to understand the layout and find where functionality lives.",
            ...
            fulltext_search: tool({
                description: 'Search the codebase for a term (e.g. "process.env."). Returns files with the term, occurrence counts, and line numbers. Use it to find env vars, integrations, and how central a symbol is.',
                inputSchema: z.object({ query: z.string().describe("The term to search for") }),
                execute: async ({ query }) => {
                    try {
        ...
        if (isOnPath("stakgraph")) {
            allTools.file_summary = tool({
                description: "Get a STRUCTURAL summary of a file (relative to the working dir): for code, its imports + every function/class signature with line ranges + call edges; for config/data files, the content. Backed by the stakgraph AST parser — prefer it over cat for understanding code files.",
                inputSchema: z.object({
        ...
        if (webSearchTool)
            allTools.web_search = webSearchTool;
        ...
        allTools.str_replace_based_edit_tool = textEditorTool
            ? textEditorTool
            : tool({
                description: "View and edit text files (sandboxed to the working dir). Commands: " +
        ...
        const model = getModel(provider, modelName ? { modelName } : undefined);
        ...
        const onStepFinish = (sf) => {
            ...
            if (sf.finishReason === "length") {
                console.warn(`[agent] TRUNCATED: generation hit maxOutputTokens=${maxOutputTokens} (finish=length, out:${sf.usage?.outputTokens ?? "?"}). A cut-off tool call never executed. Raise STRUT_MAX_OUTPUT_TOKENS or split the write.`);
            }
            if (!Array.isArray(sf.content))
                return;
        ...
        for (;;) {
            ...
            const runner = resuming
                ? new ToolLoopAgent({
                    ...
                    tools,
                    maxOutputTokens,
                    stopWhen: !useSchema && cfg.finalAnswer
            ...
            let streamError;
            await attempt.consumeStream({ onError: (e) => { streamError = e; } });
            if (!streamError) {
                res = {
                    ...
                    output: useSchema ? await attempt.output : undefined,
                };
                break;
            }
            ...
            console.warn(`[agent] stream severed after ${bankedSteps.length} banked step(s) (${streamError.message}); resuming ${streamErrorContinuations}/${MAX_STREAM_ERROR_CONTINUATIONS}.`);
        }
        ...
        console.log(`[agent] tokens in:${usage.inputTokens} cacheRead:${usage.cacheReadTokens} cacheWrite:${usage.cacheWriteTokens} out:${usage.outputTokens} → $${cost.toFixed(4)}`);
        ...
        if (useSchema) {
            ...
            if (bad.length && stepsUsed < cfg.maxSteps) {
                console.warn(`[agent] Structured answer left required field(s) empty/filler (${bad.join(", ")}) at ${stepsUsed}/${cfg.maxSteps} steps; nudging the loop once.`);
                try {
                    ...
                    const nudged = await nudger.stream({
                        messages: [
                            ...
                            {
                                role: "user",
                                content: "Your last message ended your run and was parsed as your FINAL structured answer, but it left " +
                    ...
                    let nudgeError;
                    await nudged.consumeStream({ onError: (e) => { nudgeError = e; } });
                    if (nudgeError)
                        throw nudgeError;
                    const nudgedSteps = (await nudged.steps) ?? [];
                    stepsUsed += nudgedSteps.length;
                    messages.push(...((await nudged.response)?.messages ?? []));
                    ...
                    const nudgedObject = await nudged.output;
                    ...
                    console.warn("[agent] schema nudge continuation failed:", e.message);
                }
            }
        ...
        if (cfg.finalAnswer) {
            const extractFinal = (fromSteps) => {
                for (const step of [...fromSteps].reverse()) {
                    const fa = step.content.find((c) => c.type === "tool-result" && c.toolName === "final_answer");
                    if (fa)
                        return String(fa.output ?? "");
                }
                return "";
            };
            ...
            if (classifyFinalAnswerStop(!!final, stepsUsed, cfg.maxSteps) === "nudge") {
                console.warn(`[agent] Loop ended tool-lessly at ${stepsUsed}/${cfg.maxSteps} steps without final_answer; nudging the loop once.`);
                try {
                    const nudger = new ToolLoopAgent({
                        model,
                        instructions: cfg.system,
                    ...
                    const nudged = await nudger.stream({
                        messages: [
                            ...
                            {
                                role: "user",
                                content: "You stopped by sending a message without any tool call — that ends your run, and you have NOT called final_answer. " +
                    ...
                    console.warn("[agent] nudge continuation failed:", e.message);
                }
            }
            ...
            if (!final) {
                ...
                try {
                    ...
                    let forcedError;
                    await forced.consumeStream({ onError: (e) => { forcedError = e; } });
                    if (forcedError)
                        throw forcedError;
                    ...
                    console.warn("[agent] forced final-answer turn failed:", e.message);
                }
                if (!final && lastText) {
