 ...
 */
export async function readStepSourceFromDisk(type, customDir) {
 ...
 */
async function findStepFiles(dir) {
    const files = [];
    let entries;
    try {
 ...
 */
function stepNameFromPath(filePath, baseDir) {
 ...
 */
export async function stepLoadError(filePath) {
    try {
        let suffix = "?strict";
        try {
            const { mtimeMs } = await stat(filePath);
 ...
 */
async function loadStepsFrom(baseDir, registry, sources, tier) {
 ...
 * ```
 */
export async function createRegistry(steps) {
    const registry = { ...CORE_STEPS };
    ...
    for (const def of steps) {
        if (!def || typeof def !== "object" || !def.type || !def.run) {
            throw new Error(`createRegistry: invalid step definition (missing "type" or "run")`);
        }
        ...
        if (def.type in registry) {
            ...
            console.warn(`[strut] createRegistry: user step "${def.type}" shadows built-in ${tier} step`);
        }
