...
export function capture(child, timeoutMs, maxBytes) {
    return new Promise((resolve, reject) => {
        let stdout = "";
        let stderr = "";
        let done = false;
        const cap = (s) => s.length > maxBytes ? s.slice(0, maxBytes) + "\n\n[... output truncated ...]" : s;
        const finish = (fn) => {
            if (done)
                return;
            done = true;
        ...
        child.on("close", (code) => finish(() => {
            if (code === 0)
                ...
                resolve(cap(stdout) || "No matches found");
            else
                reject(new Error(`Command failed (${code}): ${cap(stderr || stdout || "Unknown error")}`));
        }));
...
export const runCmd = (cmd, args, cwd, timeoutMs = 10000, maxBytes = 10000) => capture(spawn(cmd, args, { cwd, stdio: ["ignore", "pipe", "pipe"], env: minimalEnv() }), timeoutMs, maxBytes);
 ...
 *  not pass it. */
export const runShell = (command, cwd, timeoutMs = 15000, maxBytes = 10000, extraEnv) => capture(spawn(command, {
    cwd,
    shell: true,
