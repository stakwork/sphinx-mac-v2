...
var gatewayImageResponseSchema = z10.object({
  ...
  isRetryable: z10.boolean().optional(),
    ...
    try {
      const { responseHeaders, value: responseBody } = await postJsonToApi4({
        ...
        failedResponseHandler: createJsonErrorResponseHandler7({
          ...
          errorToMessage: (data) => {
            var _a11;
            return (_a11 = getErrorMessage7(data)) != null ? _a11 : "unknown error";
          }
        }),
        ...abortSignal && { abortSignal },
        fetch: this.config.fetch
    ...
    try {
      const {
        ...
        failedResponseHandler: createJsonErrorResponseHandler9({
          ...
          errorToMessage: (data) => {
            var _a11;
            return (_a11 = getErrorMessage9(data)) != null ? _a11 : "unknown error";
          }
        }),
        ...abortSignal && { abortSignal },
        fetch: this.config.fetch
...
var takoDataSourceInputSchema = z18.object({
  count: z18.number().optional().describe(
  ...
  include_contents: z18.boolean().optional().describe(
  ...
  mode: z18.enum(["inline", "url"]).optional().describe(
    "Requested data delivery mode. Search card data is always inline."
  ),
  content_format: z18.enum(["card_json", "csv", "json_compact", "json_records"]).optional().describe("Serialization for inlined card data."),
  max_rows: z18.number().optional().describe(
  ...
  node_ids: z18.array(z18.string()).optional().describe("Data Graph node IDs to prioritize. Maximum 20."),
  strict: z18.boolean().optional().describe(
    "Only return cards matching node_ids. Requires a non-empty node_ids."
  )
});
var takoWebSourceInputSchema = z18.object({
  count: z18.number().optional().describe("Maximum number of web results to return (1-20)."),
  include_contents: z18.boolean().optional().describe("Inline extracted web page text. This can add a data charge."),
  category: z18.enum(["finance", "news", "sports"]).optional().describe("Optional web-result category filter."),
  include_domains: z18.array(z18.string()).optional().describe("Only return results from these bare domains."),
  exclude_domains: z18.array(z18.string()).optional().describe("Exclude results from these bare domains."),
  snippet_max_chars: z18.number().optional().describe("Maximum characters in each web-result snippet."),
  highlights: z18.boolean().optional().describe(
    "Include highlighted passages in web results. Defaults to true in AI Gateway."
  ),
  article_content_max_chars: z18.number().optional().describe(
    "Maximum extracted characters per web page when including contents."
  ),
  published_after: z18.string().optional().describe("Keep results published on or after this ISO date (YYYY-MM-DD)."),
  published_before: z18.string().optional().describe(
...
var takoSearchInputSchema = lazySchema13(
  () => zodSchema13(
    z18.object({
      ...
      effort: z18.enum(["deep", "fast", "instant"]).optional().describe(
      ...
      location: z18.object({
        latitude: z18.number().describe("Latitude between -90 and 90."),
        longitude: z18.number().describe("Longitude between -180 and 180.")
      }).optional().describe("End-user coordinates for localized results."),
      country_code: z18.string().optional().describe("Two-letter ISO 3166-1 country code, such as 'US'."),
      locale: z18.string().optional().describe("BCP-47 locale, such as 'en-US'."),
      timezone: z18.string().optional().describe("IANA timezone, such as 'America/New_York'."),
      output_settings: z18.object({
        image_dark_mode: z18.boolean().optional().describe("Render card preview images in dark mode."),
        force_refresh: z18.boolean().optional().describe(
          "Instant-effort only. Request a refreshed instant result."
        )
      }).optional().describe("Controls card rendering in the search response."),
      include_related: z18.number().optional().describe("Maximum related search suggestions to include (1-20).")
    })
  )
);
var takoDatasetCellSchema = z18.union([z18.boolean(), z18.number(), z18.string()]).nullable();
var takoResultContentSchema = z18.object({
  content_format: z18.enum(["card_json", "csv", "json_compact", "json_records"]).nullish(),
  cost: z18.number().optional(),
  data: z18.string().nullish(),
  records: z18.array(z18.record(z18.string(), takoDatasetCellSchema)).nullish(),
  dataset: z18.object({
    columns: z18.array(
      z18.object({
        name: z18.string(),
        type: z18.enum(["boolean", "date", "datetime", "number", "string"]),
        unit: z18.string().nullish()
      })
    ),
    rows: z18.array(z18.array(takoDatasetCellSchema)),
    total_rows: z18.number(),
    truncated: z18.boolean(),
    ref: z18.string(),
    sources: z18.array(
      z18.object({
        name: z18.string(),
        index: z18.enum(["data", "web"]).optional()
      })
    ),
    provenance: z18.enum(["query", "web_extraction"]).optional()
  }).nullish(),
  card_data: z18.object({}).passthrough().nullish(),
  card_data_schema: z18.object({}).passthrough().nullish(),
  url: z18.string().nullish(),
  expires_at: z18.string().nullish(),
  total_rows: z18.number().nullish(),
  truncated: z18.boolean().optional(),
  export_pricing: z18.object({
    baseline_usd: z18.number(),
    free_rows: z18.number(),
    max_rows_ceiling: z18.number(),
    row_cpm_usd: z18.number()
  }).nullish(),
  manifest: z18.array(
    z18.object({
      dtype: z18.enum(["boolean", "date", "datetime", "number", "string"]).nullish(),
      entity: z18.string().nullish(),
      metric: z18.string().nullish(),
      name: z18.string().nullish(),
      unit: z18.string().nullish()
    })
  ).nullish()
}).passthrough();
var takoCardSchema = z18.object({
  card_id: z18.string().nullish(),
  title: z18.string().nullish(),
  description: z18.string().nullish(),
  semantic_description: z18.string().nullish(),
  webpage_url: z18.string().nullish(),
  image_url: z18.string().nullish(),
  embed_url: z18.string().nullish(),
  sources: z18.array(
    z18.object({
      source_name: z18.string().nullish(),
      source_description: z18.string().nullish(),
      source_index: z18.enum(["data", "web"]),
      source_text: z18.string().nullish(),
      url: z18.string().nullish()
    })
  ).nullish(),
  methodologies: z18.array(
    z18.object({
      methodology_name: z18.string().nullable(),
      methodology_description: z18.string().nullable()
    })
  ).nullish(),
  source_indexes: z18.array(z18.enum(["data", "web"])).nullish(),
  card_type: z18.string().nullish(),
  relevance: z18.enum(["High", "Low", "Medium"]).nullish(),
  content: takoResultContentSchema.nullish(),
  exportable: z18.boolean().optional(),
  nodes: z18.array(
    z18.object({
      id: z18.string(),
      type: z18.enum(["entity", "metric"]),
      name: z18.string(),
      description: z18.string().nullish()
    })
  ).nullish(),
  metric_definitions: z18.array(z18.object({ name: z18.string(), definition: z18.string() })).nullish(),
  data_freshness: z18.object({
    coverage_end: z18.string().nullish(),
    data_as_of: z18.string().nullish(),
    last_updated: z18.string().nullish()
  }).nullish()
}).passthrough();
var takoWebResultSchema = z18.object({
  title: z18.string(),
  url: z18.string(),
  snippet: z18.string().nullish(),
  source_name: z18.string().nullish(),
  publish_date: z18.string().nullish(),
  content: takoResultContentSchema.nullish()
}).passthrough();
  ...
  () => zodSchema13(
    z18.union([
      z18.object({
        ...
        cards: z18.array(takoCardSchema).optional(),
        web_results: z18.array(takoWebResultSchema).optional(),
        usage: z18.object({
          total_cost_usd: z18.number(),
          compute: z18.object({ cost_usd: z18.number() }).nullish(),
          data: z18.object({ cost_usd: z18.number(), datasets: z18.number() }).nullish()
        }).nullish(),
        related: z18.array(z18.object({}).passthrough()).nullish()
      }).passthrough(),
      z18.object({
        error: z18.enum([
          "api_error",
        ...
        statusCode: z18.number().optional(),
        message: z18.string()
      })
    ])
  )
);
var takoSearchToolFactory = createProviderToolFactoryWithOutputSchema4({
