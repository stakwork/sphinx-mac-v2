 ...
 */
export function getCategoryReveals(node, theme) {
    if (!node.category)
        return undefined;
 ...
 */
export function resolveRevealThresholds(spec, theme) {
    const td = theme.reveals;
    return {
        threshold: spec.threshold ?? td?.defaultThreshold ?? DEFAULT_THRESHOLD,
        fadeWindow: spec.fadeWindow ?? td?.defaultFadeWindow ?? DEFAULT_FADE_WINDOW,
 ...
 */
export function computeRevealOpacity(zoom, threshold, fadeWindow) {
    if (fadeWindow <= 0)
        return zoom >= threshold ? 1 : 0;
 ...
 */
function defaultRevealSize(spec, node, theme, placement) {
 ...
 */
export function computeRevealRegion(node, theme, placement, spec) {
 ...
 */
export function resolveRevealAccessor(accessor, ctx) {
 ...
 */
export const REVEAL_PLACEMENTS = [
    'above',
    'below',
    'left',
    'right',
 ...
 */
export function revealEntries(reveals) {
    const out = [];
