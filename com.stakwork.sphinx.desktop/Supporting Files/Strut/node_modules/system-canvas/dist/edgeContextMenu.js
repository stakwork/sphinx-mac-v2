 ...
 */
export function matchesEdgeContextMenuItem(item, edge, ctx) {
    const m = item.match;
    if (!m)
        return true;
    if (m.when && !m.when(edge, ctx))
        return false;
    return true;
}
/**
 ...
 */
export function filterEdgeContextMenuItems(items, edge, ctx) {
    return items.filter((item) => matchesEdgeContextMenuItem(item, edge, ctx));
}
//# sourceMappingURL=edgeContextMenu.js.map