...
export function useViewport(options) {
    ...
    const viewport = useRef(defaultViewport ?? { x: 0, y: 0, zoom: 1 });
    ...
    const fitToContent = useCallback((nodes, animate = true) => {
        const svg = svgRef.current;
    ...
    const setTransform = useCallback((transform, options) => {
        const svg = svgRef.current;
        if (!svg || !zoomBehaviorRef.current)
            return;
        const t = zoomIdentity
    ...
    const zoomToNode = useCallback((node, onComplete, options) => {
        const svg = svgRef.current;
        if (!svg || !zoomBehaviorRef.current) {
            onComplete?.();
            return;
