...
// Helpers
// ---------------------------------------------------------------------------
function makeNode(id, x, y, width, height) {
    return {
        id,
        type: 'text',
        x,
...
function makeFile(content, name = 'canvas.canvas') {
...
const testBounds = { minX: 10, minY: 20, maxX: 410, maxY: 620, width: 400, height: 600 };
...
describe('exportAsSVG', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(globalThis, 'Blob').mockImplementation(function (parts) {
            capturedSvgString = parts[0] ?? '';
            return { size: capturedSvgString.length, type: 'image/svg+xml' };
        });
    ...
    it('triggers a download (click called)', () => {
        const svg = makeSvgEl();
        exportAsSVG(svg, [makeNode('a', 0, 0, 100, 100)]);
        expect(clickSpy).toHaveBeenCalledOnce();
    });
    ...
    it('round-trip: serialised SVG string contains viewBox matching computeExportBounds', () => {
        ...
        expect(capturedSvgString.toLowerCase()).toContain(`viewbox="${bounds.minX} ${bounds.minY} ${bounds.width} ${bounds.height}"`);
    });
});
//# sourceMappingURL=exportUtils.test.js.map