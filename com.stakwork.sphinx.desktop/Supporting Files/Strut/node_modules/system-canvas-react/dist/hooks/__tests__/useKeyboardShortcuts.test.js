...
// Helpers
// ---------------------------------------------------------------------------
function makeNode(id, x = 0, y = 0, w = 100, h = 50, type = 'text') {
    return {
...
function makeNodesRef(nodes) {
    return { current: nodes };
}
function makeEdgesRef(edges) {
...
describe('useKeyboardShortcuts', () => {
    ...
    describe('Escape', () => {
        it('closes canvas context menu first (before node context menu) when open', () => {
            const opts = makeOptions({
                ...
                contextMenuState: { items: [], node: {}, screenPosition: { x: 0, y: 0 }, canvasRef: null },
                editingId: 'n1',
    ...
    describe('Arrow keys', () => {
        it('moves selected node right by grid.size on ArrowRight', () => {
            ...
            act(() => result.current(e));
            expect(opts.wrappedOnNodesUpdate).toHaveBeenCalledWith([{ id: 'n1', patch: { x: 140, y: 50 } }], undefined);
            expect(e.preventDefault).toHaveBeenCalled();
        });
        it('moves selected node left by grid.size on ArrowLeft', () => {
            ...
            expect(opts.wrappedOnNodesUpdate).toHaveBeenCalledWith([{ id: 'n1', patch: { x: 60, y: 50 } }], undefined);
        });
        it('moves selected node up by grid.size on ArrowUp', () => {
            ...
            expect(opts.wrappedOnNodesUpdate).toHaveBeenCalledWith([{ id: 'n1', patch: { x: 100, y: 10 } }], undefined);
        });
        it('uses large nudge on Shift+ArrowRight (grid.size * 10)', () => {
            ...
            expect(opts.wrappedOnNodesUpdate).toHaveBeenCalledWith([{ id: 'n1', patch: { x: 500, y: 50 } }], // 100 + 400
            undefined);
        });
    ...
    describe('Cmd+D', () => {
        ...
        it('duplicates selected nodes with offset', () => {
            ...
            const addedNode = opts.wrappedOnNodeAdd.mock.calls[0][0];
        ...
        it('preserves internal edges between duplicated nodes', () => {
            ...
            const clonedEdge = opts.wrappedOnEdgeAdd.mock.calls[0][0];
        ...
        it('calls selectMultiple with new node IDs', () => {
            ...
            const newIds = opts.selectMultiple.mock.calls[0][0];
    ...
    describe('Cmd+G', () => {
        ...
        it('creates a group node containing all selected nodes with padding', () => {
            ...
            act(() => result.current(e));
            expect(opts.wrappedOnNodeAdd).toHaveBeenCalledTimes(1);
            const group = opts.wrappedOnNodeAdd.mock.calls[0][0];
    ...
    describe('fit selection shortcut', () => {
        ...
        it('F with INPUT target does not call fitSelection and does not preventDefault', () => {
            ...
            const { result } = renderHook(() => useKeyboardShortcuts(opts));
            const e = makeKeyEvent('f', { target: { tagName: 'INPUT' } });
            act(() => result.current(e));
            expect(fitSelection).not.toHaveBeenCalled();
        ...
        it('F with TEXTAREA target does not call fitSelection and does not preventDefault', () => {
            ...
            const opts = makeOptions({ fitSelection });
            const { result } = renderHook(() => useKeyboardShortcuts(opts));
            const e = makeKeyEvent('f', { target: { tagName: 'TEXTAREA' } });
            act(() => result.current(e));
            expect(fitSelection).not.toHaveBeenCalled();
            expect(e.preventDefault).not.toHaveBeenCalled();
        });
        it('F with contentEditable target does not call fitSelection and does not preventDefault', () => {
            ...
            const opts = makeOptions({ fitSelection });
            const { result } = renderHook(() => useKeyboardShortcuts(opts));
            const e = makeKeyEvent('f', { target: { isContentEditable: true } });
            act(() => result.current(e));
            expect(fitSelection).not.toHaveBeenCalled();
            expect(e.preventDefault).not.toHaveBeenCalled();
        });
        it('Cmd+Shift+F with INPUT target still calls fitSelection (global shortcut)', () => {
            ...
            const opts = makeOptions({ fitSelection });
            const { result } = renderHook(() => useKeyboardShortcuts(opts));
            const e = makeKeyEvent('F', { metaKey: true, shiftKey: true, target: { tagName: 'INPUT' } });
            act(() => result.current(e));
            expect(fitSelection).toHaveBeenCalled();
            expect(e.preventDefault).toHaveBeenCalled();
        });
