...
import { shouldHandleSearchHotkey, useSearchHotkey, } from '../useSearchHotkey.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeCanvas() {
...
describe('useSearchHotkey', () => {
    ...
    it('calls the latest callback without re-subscribing', () => {
        const first = vi.fn();
        const second = vi.fn();
        const { rerender } = renderHook(({ cb }) => useSearchHotkey({ current: container }, cb), { initialProps: { cb: first } });
        rerender({ cb: second });
        click(appendTo(container, 'div'));
        pressCmdF(document.body);
...
//# sourceMappingURL=useSearchHotkey.test.js.map