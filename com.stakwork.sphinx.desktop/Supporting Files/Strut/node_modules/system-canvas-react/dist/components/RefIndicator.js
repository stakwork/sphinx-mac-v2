 ...
 */
export function RefIndicator({ node, theme, nodeX, nodeY, nodeWidth, nodeHeight, strokeColor, strokeWidth, corner = 'bottom-right', size: sizeProp, onNavigate, }) {
    ...
    const nodeRadius = node.type === 'group' ? theme.group.cornerRadius : node.resolvedCornerRadius;
    ...
    return (_jsxs("g", { className: "system-canvas-ref-indicator", style: { cursor: 'pointer' }, onClick: (e) => {
            e.stopPropagation();
            onNavigate(node, e);
        }, onDoubleClick: stopAll, onPointerDown: stopAll, onMouseDown: stopAll, onPointerEnter: () => setHover(true), onPointerLeave: () => setHover(false), children: [hover && (_jsxs(_Fragment, { children: [_jsx("defs", { children: _jsx("clipPath", { id: clipId, children: _jsx("rect", { x: nodeX, y: nodeY, width: nodeWidth, height: nodeHeight, rx: nodeRadius }) }) }), _jsx("path", { d: hoverPath, fill: hoverFill, clipPath: `url(#${clipId})`, pointerEvents: "none" })] })), _jsx("path", { d: carvePath, fill: "none", stroke: strokeColor, strokeWidth: sw, strokeLinecap: "butt", strokeLinejoin: "round", pointerEvents: "none" }), _jsx("rect", { x: squareX, y: squareY, width: size, height: size, fill: "transparent" }), _jsx(Glyph, { kind: iconKind, cx: cx, cy: cy, color: hover ? hoverGlyph : restGlyph, size: size })] }));
}
function Glyph({ kind, cx, cy, color, size, }) {
    ...
    switch (kind) {
        case 'chevron':
            return (_jsx("path", { d: `M ${cx - s / 2} ${cy - s} L ${cx + s / 2} ${cy} L ${cx - s / 2} ${cy + s}`, fill: "none", stroke: color, strokeWidth: sw, strokeLinecap: "round", strokeLinejoin: "round", pointerEvents: "none" }));
        case 'arrow': {
            const h = s * 0.9; // head size
            return (_jsx("path", { d: `M ${cx - s} ${cy} L ${cx + s} ${cy} M ${cx + s - h} ${cy - h} L ${cx + s} ${cy} L ${cx + s - h} ${cy + h}`, fill: "none", stroke: color, strokeWidth: sw, strokeLinecap: "round", strokeLinejoin: "round", pointerEvents: "none" }));
        }
        case 'expand': {
            const sw2 = Math.max(1, size / 12);
            return (_jsxs("g", { pointerEvents: "none", children: [_jsx("path", { d: `M ${cx - s} ${cy - s} L ${cx + s} ${cy - s} L ${cx + s} ${cy + s} L ${cx - s} ${cy + s} Z`, fill: "none", stroke: color, strokeWidth: sw2, strokeLinejoin: "round" }), _jsx("line", { x1: cx, y1: cy - s + 1, x2: cx, y2: cy + s - 1, stroke: color, strokeWidth: sw2, strokeLinecap: "round" }), _jsx("line", { x1: cx - s + 1, y1: cy, x2: cx + s - 1, y2: cy, stroke: color, strokeWidth: sw2, strokeLinecap: "round" })] }));
        }
    }
}
//# sourceMappingURL=RefIndicator.js.map