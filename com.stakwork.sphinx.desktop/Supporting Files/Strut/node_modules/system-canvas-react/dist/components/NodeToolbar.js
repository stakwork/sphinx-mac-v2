...
import { canvasToScreen, filterActionsForNode, getNodeActionsForNode, resolveActionPatch, } from 'system-canvas';
 ...
 */
export function NodeToolbar({ node, theme, onPatch, onDelete, getViewport, containerWidth, containerHeight, render, selectedNodes, onMultiPatch, onAlign, onDistribute, }) {
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            setSize((prev) => prev.width === r.width && prev.height === r.height
                ? prev
    ...
    return (_jsxs("div", { ref: toolbarRef, className: "system-canvas-node-toolbar", 
        // Stop pointer events from bubbling to the canvas (which would
        // deselect the node or start a pan).
        onPointerDown: (e) => e.stopPropagation(), onMouseDown: (e) => e.stopPropagation(), onClick: (e) => e.stopPropagation(), onDoubleClick: (e) => e.stopPropagation(), style: {
            position: 'absolute',
            left,
            top,
            display: 'flex',
        ...
        }, children: [_jsx("style", { children: `@keyframes system-canvas-toolbar-in { from { opacity: 0; transform: translateY(4px) } to { opacity: 1; transform: translateY(0) } }` }), isMulti && selectedNodes && onMultiPatch ? (_jsx(MultiToolbarContent, { selectedNodes: selectedNodes, theme: theme, onMultiPatch: onMultiPatch, onDelete: deleteNode, onAlign: onAlign, onDistribute: onDistribute })) : render ? (render({ node, theme, patch, deleteNode })) : (_jsx(DefaultToolbarContent, { node: node, theme: theme, onPatch: patch, onDelete: deleteNode }))] }));
}
function MultiToolbarContent({ selectedNodes, theme, onMultiPatch, onDelete, onAlign, onDistribute, }) {
    ...
    const groups = useMemo(() => getNodeActionsForNode(representativeNode, theme), [representativeNode, theme]);
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("span", { style: {
                    fontSize: 11,
                    color: theme.breadcrumbs.textColor,
                    opacity: 0.75,
                ...
                if (actions.length === 0)
                    return null;
                return (_jsxs(React.Fragment, { children: [i > 0 && _jsx(Divider, { theme: theme }), _jsx("div", { style: { display: 'flex', alignItems: 'center', gap: BUTTON_GAP }, children: actions.map((action) => {
                                const active = action.isActive?.(representativeNode) ?? false;
                                const handleClick = () => {
                                    const patch = resolveActionPatch(action, representativeNode);
                                    onMultiPatch(patch);
                                };
                                return (_jsx(SwatchButton, { action: action, active: active, theme: theme, onClick: handleClick }, action.id));
                            }) })] }, group.id));
            }), otherGroups.map((group) => {
                const actions = filterActionsForNode(group, representativeNode);
                if (actions.length === 0)
                    return null;
                return (_jsxs(React.Fragment, { children: [_jsx(Divider, { theme: theme }), _jsx("div", { style: { display: 'flex', alignItems: 'center', gap: BUTTON_GAP }, children: actions.map((action) => {
                                const active = action.isActive?.(representativeNode) ?? false;
                                const handleClick = () => {
                                    const patch = resolveActionPatch(action, representativeNode);
                                    onMultiPatch(patch);
                                };
                                return (_jsx(IconButton, { action: action, active: active, theme: theme, onClick: handleClick }, action.id));
                            }) })] }, group.id));
            }), onAlign && (_jsxs(_Fragment, { children: [_jsx(Divider, { theme: theme }), _jsx("div", { style: { display: 'flex', alignItems: 'center', gap: BUTTON_GAP }, children: [
                            ...
                            { dir: 'centerV', label: 'Center Vertically', path: 'M 3 8 L 13 8 M 6 5 L 6 11 M 10 4 L 10 12' },
                        ].map(({ dir, label, path }) => (_jsx("button", { type: "button", title: label, onClick: () => onAlign(dir), onMouseDown: (e) => e.preventDefault(), style: {
                                width: BUTTON_SIZE,
                                height: BUTTON_SIZE,
                                display: 'inline-flex',
                                alignItems: 'center',
                                ...
                                padding: 0,
                                outline: 'none',
                            }, children: _jsx("svg", { width: 16, height: 16, viewBox: "0 0 16 16", children: _jsx("path", { d: path, fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }) }) }, dir))) })] })), onDistribute && (_jsxs(_Fragment, { children: [_jsx(Divider, { theme: theme }), _jsx("div", { style: { display: 'flex', alignItems: 'center', gap: BUTTON_GAP }, children: ([
                            ...
                            return (_jsx("button", { type: "button", title: label, disabled: disabled, onClick: () => !disabled && onDistribute(axis), onMouseDown: (e) => e.preventDefault(), style: {
                                    width: BUTTON_SIZE,
                                    height: BUTTON_SIZE,
                                    display: 'inline-flex',
                                    alignItems: 'center',
                                    ...
                                    padding: 0,
                                    outline: 'none',
                                }, children: _jsx("svg", { width: 16, height: 16, viewBox: "0 0 16 16", children: _jsx("path", { d: path, fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }) }) }, axis));
                        }) })] })), showDelete && (_jsxs(_Fragment, { children: [_jsx(Divider, { theme: theme }), _jsx(DeleteButton, { theme: theme, onDelete: onDelete })] }))] }));
}
function DefaultToolbarContent({ node, theme, onPatch, onDelete, }) {
    ...
    const groups = useMemo(() => getNodeActionsForNode(node, theme), [node, theme]);
    const showDelete = theme.showToolbarDelete === true;
    return (_jsxs(_Fragment, { children: [groups.map((group, i) => (_jsxs(React.Fragment, { children: [i > 0 && _jsx(Divider, { theme: theme }), _jsx(ActionGroupView, { group: group, node: node, theme: theme, onPatch: onPatch })] }, group.id))), showDelete && (_jsxs(_Fragment, { children: [_jsx(Divider, { theme: theme }), _jsx(DeleteButton, { theme: theme, onDelete: onDelete })] }))] }));
}
...
function ActionGroupView({ group, node, theme, onPatch }) {
    const actions = filterActionsForNode(group, node);
    if (actions.length === 0)
        return null;
    const kind = group.kind ?? 'buttons';
    if (kind === 'menu') {
        return _jsx(MenuGroup, { group: group, actions: actions, node: node, theme: theme, onPatch: onPatch });
    }
    return (_jsx("div", { title: group.label, style: { display: 'flex', alignItems: 'center', gap: BUTTON_GAP }, children: actions.map((action) => {
            const handleClick = () => {
...
function SwatchButton({ action, active, theme, onClick, }) {
    ...
    return (_jsx("button", { type: "button", title: action.label, onClick: onClick, onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), style: {
            width: SWATCH_SIZE,
            height: SWATCH_SIZE,
            borderRadius: '50%',
            background: color,
    ...
    return (_jsx("button", { type: "button", title: action.label, onClick: onClick, onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), style: {
            ...
            background: active
                ...
                : hovered
                    ...
                    : 'transparent',
            border: `1px solid ${active ? theme.breadcrumbs.activeColor : theme.breadcrumbs.separatorColor}`,
            borderRadius: 6,
            color: active
        ...
        }, onMouseDown: (e) => e.preventDefault(), children: action.icon ? (_jsx("svg", { width: 16, height: 16, viewBox: "0 0 16 16", children: _jsx(NodeIcon, { icon: action.icon, x: 0, y: 0, size: 16, color: active
                    ? theme.breadcrumbs.activeColor
                    : theme.breadcrumbs.textColor, opacity: 1, customIcons: theme.icons }) })) : action.swatch ? (_jsx("span", { style: {
                width: 10,
                height: 10,
                borderRadius: '50%',
                background: action.swatch,
            } })) : (_jsx("span", { style: { fontSize: 10 }, children: action.label.slice(0, 2) })) }));
}
function MenuGroup({ group, actions, node, theme, onPatch }) {
    ...
    return (_jsxs("div", { ref: wrapRef, style: { position: 'relative' }, children: [_jsxs("button", { type: "button", title: group.label, onClick: () => setOpen((v) => !v), style: {
                    height: BUTTON_SIZE,
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: 6,
                    ...
                    fontSize: 'inherit',
                    outline: 'none',
                }, onMouseDown: (e) => e.preventDefault(), children: [triggerIcon && (_jsx("svg", { width: 14, height: 14, viewBox: "0 0 16 16", children: _jsx(NodeIcon, { icon: triggerIcon, x: 0, y: 0, size: 14, color: theme.breadcrumbs.textColor, opacity: 1, customIcons: theme.icons }) })), _jsx("span", { children: triggerLabel }), _jsx("span", { style: { opacity: 0.6, fontSize: 8 }, children: "\u25BE" })] }), open && (_jsx("div", { style: {
                    position: 'absolute',
                    top: '100%',
                    left: 0,
                    marginTop: 4,
                    ...
                    return (_jsxs("button", { type: "button", onClick: () => {
                            ...
                            fontSize: 'inherit',
                            textAlign: 'left',
                        }, onMouseDown: (e) => e.preventDefault(), children: [action.icon && (_jsx("svg", { width: 14, height: 14, viewBox: "0 0 16 16", children: _jsx(NodeIcon, { icon: action.icon, x: 0, y: 0, size: 14, color: isActive
                                        ? theme.breadcrumbs.activeColor
                                        : theme.breadcrumbs.textColor, opacity: 1, customIcons: theme.icons }) })), action.swatch && (_jsx("span", { style: {
                                    width: 10,
                                    height: 10,
                                    ...
                                    background: action.swatch,
                                    flexShrink: 0,
                                } })), _jsx("span", { children: action.label })] }, action.id));
                }) }))] }));
}
function DeleteButton({ theme, onDelete, }) {
    return (_jsx("button", { type: "button", title: "Delete", onClick: onDelete, onMouseDown: (e) => e.preventDefault(), style: {
            width: BUTTON_SIZE,
            height: BUTTON_SIZE,
            display: 'inline-flex',
            alignItems: 'center',
            ...
            padding: 0,
            outline: 'none',
        }, children: _jsx("svg", { width: DELETE_SIZE, height: DELETE_SIZE, viewBox: "0 0 16 16", children: _jsx("path", { d: "M 3 5 L 13 5 M 6 5 L 6 3 L 10 3 L 10 5 M 5 5 L 5.5 14 L 10.5 14 L 11 5 M 7 7 L 7 12 M 9 7 L 9 12", fill: "none", stroke: "currentColor", strokeWidth: 1.2, strokeLinecap: "round", strokeLinejoin: "round" }) }) }));
}
//# sourceMappingURL=NodeToolbar.js.map