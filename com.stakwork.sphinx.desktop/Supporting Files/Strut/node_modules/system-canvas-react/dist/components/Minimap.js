 ...
 */
export function computeUnionedBounds(nodeBounds, viewport) {
 ...
 */
export function computeAvgNodeWidth(nodes) {
    if (nodes.length === 0)
        return 0;
 ...
 * groups.
 */
export function bucketNodesIntoDensityGrid(nodes, bounds) {
    ...
    for (const node of nodes) {
        ...
        const col = Math.min(GRID_SIZE - 1, Math.max(0, Math.floor(((cx - bounds.minX) / bw) * GRID_SIZE)));
        const row = Math.min(GRID_SIZE - 1, Math.max(0, Math.floor(((cy - bounds.minY) / bh) * GRID_SIZE)));
 ...
 */
export function applyHysteresis(current, value, enterThreshold, exitThreshold) {
    if (current === 'dots' && value < enterThreshold)
        return 'grid';
    if (current === 'grid' && value > exitThreshold)
        return 'dots';
    return current;
}
// ---------------------------------------------------------------------------
// Component
// ---------------------------------------------------------------------------
export function Minimap({ nodes, theme, getViewport, setTransform, getSvgElement, width = 160, height = 120, }) {
    ...
    const unionedBoundsRef = useRef(null);
    // Hysteresis state for dots/grid render mode
    const renderModeRef = useRef('dots');
    // ---------------------------------------------------------------------------
    // Coordinate helpers
    // ---------------------------------------------------------------------------
    const toMinimap = useCallback((cx, cy, bounds) => {
        const bw = bounds.width || 1;
        const bh = bounds.height || 1;
    ...
    const fromMinimap = useCallback((mx, my, bounds) => {
        const bw = bounds.width || 1;
        const bh = bounds.height || 1;
        const scale = Math.min((width - PAD * 2) / bw, (height - PAD * 2) / bh);
        const ox = (width - bw * scale) / 2;
    ...
    const paint = useCallback(() => {
        ...
        renderModeRef.current = applyHysteresis(renderModeRef.current, avgNodeWidth * scale, 3, 4);
    ...
    if (nodes.length === 0)
        return null;
    return (_jsx("div", { ref: containerRef, className: "system-canvas-minimap", onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerUp, onPointerEnter: () => setHovered(true), onPointerLeave: () => {
            setHovered(false);
            draggingRef.current = false;
        }, style: {
            position: 'absolute',
