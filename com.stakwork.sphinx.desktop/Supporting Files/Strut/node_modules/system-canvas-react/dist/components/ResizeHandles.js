 ...
 */
const cornerInset = (cornerRadius) => Math.min(cornerRadius * 0.25, 3);
/**
 ...
 */
export function ResizeHandles({ node, theme, onHandlePointerDown, }) {
    ...
    return (_jsxs("g", { className: "system-canvas-resize-handles", pointerEvents: "all", onPointerEnter: () => setGroupHovered(true), onPointerLeave: () => setGroupHovered(false), children: [_jsx("rect", { x: x - HANDLE_SIZE, y: y - HANDLE_SIZE, width: width + HANDLE_SIZE * 2, height: height + HANDLE_SIZE * 2, fill: "none", stroke: "transparent", strokeWidth: HANDLE_SIZE * 2, pointerEvents: "stroke" }), CORNERS.map(({ corner, cursor, anchor }) => {
                const { cx, cy } = anchorPos(anchor);
                const isHovered = hoveredCorner === corner;
                const s = isHovered ? HANDLE_SIZE + 2 : HANDLE_SIZE;
                const half = s / 2;
                return (_jsx("rect", { x: cx - half, y: cy - half, width: s, height: s, fill: handleColor, opacity: resizing ? 0 : (groupHovered || isHovered ? 1 : 0), style: {
                        cursor,
                        transition: 'opacity 120ms ease-out',
                    }, onPointerEnter: () => setHoveredCorner(corner), onPointerLeave: () => setHoveredCorner((c) => (c === corner ? null : c)), onPointerDown: (e) => {
                        setResizing(true);
                        const onUp = () => {
                            setResizing(false);
                    ...
                    }, onClick: (e) => e.stopPropagation(), onDoubleClick: (e) => e.stopPropagation() }, corner));
            })] }));
}
