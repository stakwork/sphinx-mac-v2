...
describe('CanvasContextMenuOverlay', () => {
    it('renders nothing when state is null', () => {
        ...
        const { container } = render(_jsx(CanvasContextMenuOverlay, { state: null, config: config, theme: makeTheme(), onClose: vi.fn() }));
        expect(container.firstChild).toBeNull();
    });
    it('renders all items when state is set', () => {
        const state = makeState();
        const { container } = render(_jsx(CanvasContextMenuOverlay, { state: state, config: makeConfig(), theme: makeTheme(), onClose: vi.fn() }));
    ...
    it('calls config.onSelect with correct itemId and context on item click, then calls onClose', () => {
        ...
        render(_jsx(CanvasContextMenuOverlay, { state: state, config: makeConfig({ onSelect }), theme: makeTheme(), onClose: onClose }));
    ...
    it('calls onClose on Escape keydown', () => {
        const onClose = vi.fn();
        const state = makeState();
        render(_jsx(CanvasContextMenuOverlay, { state: state, config: makeConfig(), theme: makeTheme(), onClose: onClose }));
    ...
    it('does NOT call onSelect for the __sep__ sentinel', () => {
        ...
        const { container } = render(_jsx(CanvasContextMenuOverlay, { state: state, config: makeConfig({ onSelect }), theme: makeTheme(), onClose: onClose }));
    ...
    it('renders nothing when theme has no contextMenu block', () => {
        const state = makeState();
        const { container } = render(_jsx(CanvasContextMenuOverlay, { state: state, config: makeConfig(), theme: {}, onClose: vi.fn() }));
        expect(container.firstChild).toBeNull();
    });
});
//# sourceMappingURL=CanvasContextMenuOverlay.test.js.map