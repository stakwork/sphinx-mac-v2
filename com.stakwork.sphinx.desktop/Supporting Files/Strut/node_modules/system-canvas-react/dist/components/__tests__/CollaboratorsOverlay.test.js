...
vi.mock('system-canvas', async (importOriginal) => {
    ...
    return {
        ...actual,
        canvasToScreen: vi.fn((cx, cy, vp) => ({
            x: cx * vp.zoom + vp.x,
            y: cy * vp.zoom + vp.y,
        })),
        canvasRectToScreenRect: vi.fn((rect, vp) => ({
...
// Helpers
// ---------------------------------------------------------------------------
function makeNode(id, x = 0, y = 0, width = 200, height = 100) {
    return {
        id,
        type: 'text',
        x,
...
const VIEWPORT_2X = { x: 10, y: 20, zoom: 2 };
// ---------------------------------------------------------------------------
...
describe('CollaboratorsOverlay', () => {
    ...
    it('renders nothing when collaborators is empty and no flash', () => {
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [], viewport: VIEWPORT, nodeMap: makeNodeMap(), flashNodeIds: new Map() }));
    ...
    it('overlay container has pointer-events: none', () => {
        ...
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(), flashNodeIds: new Map() }));
    ...
    it('renders a cursor element for a collaborator with a non-null cursor', () => {
        ...
        render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(), flashNodeIds: new Map() }));
    ...
    it('positions cursor at screen-space coordinates derived from viewport', () => {
        ...
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT_2X, nodeMap: makeNodeMap(), flashNodeIds: new Map() }));
        // Find the cursor container div
        const overlay = container.firstChild;
        const cursorEl = overlay.querySelector('[style*="left: 110px"]');
    ...
    it('renders a halo around a node that a collaborator has selected', () => {
        ...
        render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(node), flashNodeIds: new Map() }));
    ...
    it('uses collaborator color for the halo border', () => {
        ...
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(node), flashNodeIds: new Map() }));
        // Border div should include the collaborator color
        const overlay = container.firstChild;
        const borderDiv = overlay.querySelector('[style*="border: 2px solid"]');
        ...
        const hasBorderColor = borderStyle.includes('#aa00bb') || borderStyle.includes('rgb(170, 0, 187)');
    ...
    it('stacks halos for multiple collaborators selecting the same node', () => {
        ...
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: collaborators, viewport: VIEWPORT, nodeMap: makeNodeMap(node), flashNodeIds: new Map() }));
        const overlay = container.firstChild;
    ...
    it('renders a flash overlay for a nodeId present in flashNodeIds', () => {
        ...
        const flashNodeIds = new Map([['flash-node', Date.now() + 600]]);
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [], viewport: VIEWPORT, nodeMap: makeNodeMap(node), flashNodeIds: flashNodeIds }));
        const overlay = container.firstChild;
        expect(overlay).not.toBeNull();
    ...
    it('does not render flash when flashNodeIds is empty', () => {
        const { container } = render(_jsx(CollaboratorsOverlay, { collaborators: [], viewport: VIEWPORT, nodeMap: makeNodeMap(makeNode('n1')), flashNodeIds: new Map() }));
        expect(container.firstChild).toBeNull();
    });
    it('does not crash when collaborators is empty', () => {
        expect(() => render(_jsx(CollaboratorsOverlay, { collaborators: [], viewport: VIEWPORT, nodeMap: makeNodeMap(), flashNodeIds: new Map() }))).not.toThrow();
    });
    // -------------------------------------------------------------------------
    // Clip guard
    // -------------------------------------------------------------------------
    it('renders cursor within bounds when containerWidth/Height are provided', () => {
        ...
        render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(), flashNodeIds: new Map(), containerWidth: 200, containerHeight: 200 }));
        expect(screen.getAllByText('Alice').length).toBeGreaterThanOrEqual(1);
    });
    ...
    it('halo layer is unaffected by container size props', () => {
        ...
        render(_jsx(CollaboratorsOverlay, { collaborators: [collab], viewport: VIEWPORT, nodeMap: makeNodeMap(node), flashNodeIds: new Map(), containerWidth: 10, containerHeight: 10 }));
        // Halo label still renders even with tiny container
        expect(screen.getByText('HaloUser')).toBeTruthy();
    });
});
//# sourceMappingURL=CollaboratorsOverlay.test.js.map