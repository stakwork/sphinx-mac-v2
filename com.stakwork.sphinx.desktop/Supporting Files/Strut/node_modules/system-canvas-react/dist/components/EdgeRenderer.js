 ...
 */
export function EdgeRenderer({ edges, nodeMap, theme, defaultEdgeStyle, onClick, onDoubleClick, onContextMenu, selectedId, editingId, dimmedNodeIds, parallelGroups, editable, onWaypointCommit, viewportRef, }) {
    // Stable fallback ref so the hook is always called unconditionally.
    const fallbackViewportRef = React.useRef({ x: 0, y: 0, zoom: 1 });
    const effectiveViewportRef = (viewportRef ?? fallbackViewportRef);
    const { overrides, onHandlePointerDown, onHandleDoubleClick, onGhostClick } = useWaypointDrag({
        viewport: effectiveViewportRef,
        onCommit: onWaypointCommit ?? (() => { }),
    });
    const knownEdgeIdsRef = useRef(new Set());
    const newEdgeIdsRef = useRef(new Set());
    useEffect(() => {
        const currentIds = new Set(edges.map((e) => e.id));
        const fresh = new Set();
        for (const id of currentIds) {
            if (!knownEdgeIdsRef.current.has(id))
                fresh.add(id);
        }
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("defs", { children: [_jsx("style", { children: `
        ...
        ` }), _jsx("marker", { id: "system-canvas-arrowhead", markerWidth: theme.edge.arrowSize, markerHeight: theme.edge.arrowSize * 0.7, refX: theme.edge.arrowSize - 1, refY: theme.edge.arrowSize * 0.35, orient: "auto", markerUnits: "userSpaceOnUse", children: _jsx("polygon", { points: `0 0, ${theme.edge.arrowSize} ${theme.edge.arrowSize * 0.35}, 0 ${theme.edge.arrowSize * 0.7}`, fill: "context-stroke" }) })] }), [...edges]
                ...
                return (_jsxs("g", { className: "system-canvas-edge", style: { cursor: 'pointer' }, onClick: (e) => onClick(edge, e), onDoubleClick: (e) => onDoubleClick?.(edge, e), onContextMenu: (e) => onContextMenu(edge, e), children: [_jsx("path", { d: pathD, fill: "none", stroke: "transparent", strokeWidth: 12 }), isNewEdge && !edge.animated ? (_jsx(AnimatedEdgePath, { d: pathD, stroke: edgeColor, strokeWidth: strokeWidth, strokeDasharray: isEdgeDimmed ? '4 4' : undefined, strokeOpacity: isEdgeDimmed ? 0.3 : 1, markerEnd: toEnd === 'arrow' ? `url(#${arrowId})` : undefined, markerStart: fromEnd === 'arrow' ? `url(#${arrowId})` : undefined })) : (_jsx("path", { d: pathD, fill: "none", stroke: edgeColor, strokeWidth: strokeWidth, strokeDasharray: edge.animated ? '6 4' : (isEdgeDimmed ? '4 4' : undefined), style: edge.animated ? { animation: 'system-canvas-march 0.6s linear infinite' } : undefined, strokeOpacity: isEdgeDimmed ? 0.3 : 1, markerEnd: toEnd === 'arrow' ? `url(#${arrowId})` : undefined, markerStart: fromEnd === 'arrow' ? `url(#${arrowId})` : undefined })), edge.label && !isEditing && (_jsx(_Fragment, { children: (() => {
                                ...
                                return (_jsxs(_Fragment, { children: [_jsx("rect", { x: pillX, y: pillY, width: pillW, height: pillH, rx: pillH / 2, fill: theme.background, opacity: 0.85 }), _jsx("text", { x: midpoint.x, y: pillY + pillH / 2 + labelFontSize * 0.35, fill: isSelected ? theme.node.labelColor : theme.edge.labelColor, fontSize: labelFontSize, fontFamily: theme.node.fontFamily, textAnchor: "middle", pointerEvents: "none", children: edge.label })] }));
                            })() })), editable && isSelected && (() => {
                            const baseWaypoints = edge.waypoints ?? [];
                            // Apply live drag overrides
                            const liveWaypoints = baseWaypoints.map((wp, i) => overrides.has(i) ? overrides.get(i) : wp);
                            ...
                            return (_jsxs(_Fragment, { children: [ghostPoints.map((gp, i) => (_jsx("circle", { cx: gp.x, cy: gp.y, r: 4, fill: theme.edge.stroke, opacity: 0.35, style: { cursor: 'crosshair' }, onClick: (e) => {
                                            e.stopPropagation();
                                            onGhostClick(edge.id, liveWaypoints, gp.insertAfterIndex, { x: gp.x, y: gp.y });
                                        } }, `ghost-${i}`))), liveWaypoints.map((wp, i) => (_jsx("circle", { cx: wp.x, cy: wp.y, r: 5, fill: theme.node.labelColor, stroke: theme.edge.stroke, strokeWidth: 1.5, style: { cursor: 'grab' }, onPointerDown: (e) => onHandlePointerDown(edge.id, liveWaypoints, i, e), onDoubleClick: (e) => {
                                            e.stopPropagation();
...
function AnimatedEdgePath({ d, stroke, strokeWidth, strokeDasharray, strokeOpacity, markerEnd, markerStart, }) {
    ...
    return (_jsx("path", { ref: pathRef, d: d, fill: "none", stroke: stroke, strokeWidth: strokeWidth, strokeOpacity: strokeOpacity, markerEnd: markerEnd, markerStart: markerStart }));
}
